...
x = c("initialize", "get_xi_upper", "get_xi_lower", "get_bifurcatable",
	"get_regions", "rejection_bound", "nc", "r", "d", "log_target_pdf_unnorm",
	"summary", "print", "clone")
stopifnot(setequal(names(FMMProposal$public_methods), x))
```
...
- Methods `get_xi_upper`, `get_xi_lower`, `get_bifurcatable`, and
  `get_regions` are accessors for private data members: `get_xi_upper` returns
  $\overline{\xi}_1, \ldots, \overline{\xi}_N$; `get_xi_lower` returns
  $\underline{\xi}_1, \ldots, \underline{\xi}_N$; `get_bifurcatable` returns a
  vector of logical values indicating whether respective regions can be
  bifurcated (described further in Section TBD); and `get_regions` returns the
  list of `Region` objects on which the `FMMProposal` was constructed.
...
#| include: false
# If the interface has changed, catch it and bail out
x = c("d_base", "w", "r", "d", "s", "w_major", "bifurcate", "is_bifurcatable",
	"xi_upper", "xi_lower", "description", "print", "clone")
stopifnot(setequal(names(Region$public_methods), x))
```
...
TBD: Show a typical workflow, maybe something abstract that doesn't evaluate...
