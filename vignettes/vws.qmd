...
<!-- -->
\begin{displaymath}
e^{-\lambda} \sum_{x=0}^{n-1} \frac{\lambda^x}{x!}
= \frac{\Gamma(n, \lambda)}{\Gamma(n)},
\quad n \in \{ 1, 2, \ldots \},
\end{displaymath}
<!-- -->
where $\Gamma(c,z) = \int_z^\infty t^{c-1} e^{-t} dt$ and $\Gamma(c) = \Gamma(c,0)$. Therefore, for $a, b \in \mathbb{R}$ with $0 \leq a \leq b$ and $X \sim \text{Poisson}(\lambda)$,
<!-- -->
\begin{align*}
\Prob(a < X \leq b)
