...
- The `adapt` function partitions the proposal to make it better
...
1. (DONE) High level description of the design
	a. `FMMProposal`
	b. `rejection`
	c. `adapt`
	d. Other stuff at a high level
...
Before sampling, the function `adapt` can be used to adapt a given
`FMMProposal` object by partitioning the initial set of regions into a finer
set. In the following example, a proposal `h_init` is adapted `N = 10` times
...
adapt_out = adapt(h_init, N = 30, control = ctrl)
h = adapt_out$h
