...
2.  Make sure to check for other TBDs that may be scattered throughout.
:::
...
4. Construct a region of type `MyRegion` which we have presumably defined in `MyRegion.h`. The symbol `...` here isz a placeholder for any arguments needed by the constructor.
...
8. Pack up elements of the struct `out` into an `Rcpp::List` and return them to the caller.
...
The following typedef is used to represent functions that take `double` and `boolean` arguments and return a `double`. It is used as the type for weight functions in particular, with `x` the argument of the weight function and `log = true` if the result is to be returned on the log-scale.
...
`RealConstRegion` has the following constructors.
...
The first constructor creates a region based on the interval $(a,b]$; the second creates a region based on the singleton set $\{a\}$, which is intended primarily for internal use. Note that `a` and `b` need not be integers.
...
4. The action to take if `max_rejects` rejections is exceeded. The definition of the `fntl::error_action` enum is given in Section 2 of @fntl.
...
If `action = fntl::error_action::STOP`, the `rejection` functions halts when an error condition is encountered. If `action = fntl::error_action::WARNING` or `action = fntl::error_action::MESSAGE`, a warning or message is emitted without halting. Otherwise, if `action = fntl::error_action::NONE`, the error condition is ignored.

before all $n$ desired variates are drawn. In this case, the `draws` and `rejects` vectors will have lengths shorter than $n$.
...
Jointly draw values of $Y$ and $Z$ from the model; $Z$ is considered observed while $Y$ is latent and the objective for inference.
