...
- We can consider suppressing plotting code from the document if it gets
  too lengthy.
...
- Clean out experimental stuff and maybe put it somewhere else.
:::
...
**TBD: write about methods here**

# Programming Preliminaries {#sec-prelim}

## Lambdas
## fntl Package

Mention density, cdf, quantile function definitions and numerical tools.

## Exposing Functions to R

The model of coding in C++ and exposing functions to R

## Log-Scale

Explain that computations are kept on log-scale in the package and why we do it.

## VWS Namespace

Perhaps this should be pointed out in the first C++ example instead.
...
Here, `region1` and `region2` are objects whose class is a subclass of `Region`. The `Region` class itself is abstract and defines necessary operations. Further details are given in Sections [-@sec-region], [-@sec-cpp-api-realconstregion], and [-@sec-user-region].
...
## Quick Start in R {#sec-quick-r}

::: {.callout-caution title="TBD"}
Under construction. Describe the minimum we need to know to understand this
example. Refer to details, which I think will be later in the document.
:::

```{.cpp include="examples/first-r/sample.R" code-line-numbers="true"}
```


```{r}
source("examples/first-r/sample.R")
sample(n = 10, kappa = 5, d = 4, N = 10)
```

```{.cpp include="examples/first-r/sample2.R" code-line-numbers="true"}
```

```{r}
source("examples/first-r/sample2.R")
sample2(n = 10, kappa = 5, d = 4, N = 10)
```

```{r}
# TEMP code to compute max of weight function.
...
curve(w(x), xlim = c(-1,1))
abline(v = x_roots, lty = 2)

curve(f, xlim = c(-1,1))
abline(h = 0, lty = 2)
```

## Quick Start in C++ {#sec-first-cpp}
...
declares a `sample` function in C++ that is also exposed as an R function.

```{.cpp include="examples/first-cpp/sample.cpp" code-line-numbers="true"}
```
...
- Lines 22-24 construct a specific type of `Region` and build an `FMMProposal`
  ...
  `UnivariateConstRegion` specifies the type of `Region`.
...
```{r}
Rcpp::sourceCpp("examples/first-cpp/sampler.cpp")
sample(n = 10, kappa = 5, d = 4, N = 10)
```
...
## (STALE) Refining the Proposal {#sec-adapt}
...
## (STALE) User-Defined Regions {#sec-user-region}
...
**TBD**: consider flattening the structure of this section.

An implementation of VWS rejection sampling is provided in R to help with quick
prototyping. It is limited to univariate problems, and assumes constant
majorizers and minorizers where constants are obtained from numerical
optimization.

## Rejection Sampling {#sec-r-rejection}

TBD: there are functions for numeric draws and integer draws.
...
```
1. Number of desired variates.
2. Lower endpoint of support; may be `-Inf`.
3. Upper endpoint of support; may be `Inf`.
4. A weight function `w(x, log)` which takes argument: `x` a point in the
support and `log` an indicator of whether value should be returned on the
log-scale.
5. A structure created from `univariate_helper` (@sec-r-univariate-helper).
6. An object constructed using `rejection_control` (@sec-r-rejection-control).

## Rejection Control {#sec-r-rejection-control}
...
rejection_control(
	N = 30,                                           # <1>
	tol = 0.1,                                        # <2>
	max_rejects = .Machine$integer.max,               # <3>
	report = .Machine$integer.max,                    # <4>
	ratio_ub = exp(1e-5),                             # <5>
	action = c("stop", "warning", "message", "none"), # <6>
	maxopt = NULL,                                    # <7>
	minopt = NULL                                     # <8>
)
```
1. Maximum number of regions to create in the proposal.
2. Desired bound for probability of rejection in the proposal. The default
value of `0` ensures that the proposal is adapted `N` times without halting
early.
3. Maximum number of rejections to tolerate before bailing out.
4. Report progress each time this many candidates are proposed.
5. Rejection ratio $f_0(x) / h_0(x)$ may be larger than 1 due to numerical
precision. Throw an error if it is larger than this value.
6. Action to take if rejection sampling halts early due to the maximum number
of rejections being exceeded.
7. A maximization routine. See below.
8. A minimization routine. See below.

The arguments `maxopt` and `minopt` are functions of the form `function(w, lo, hi, log)` that repectively maximize and minimize the weight function `w` on the interval `(lo, hi]`. If `log = T`, the optimized value of `log(w(x))` is returned (on the log-scale); otherwise, the optimized value of `w(x)` is returned. If these are left to the default `NULL` value, an numerical optimization routine will be used.

**TBD**: point to numerical optimization.

## Univariate Helper {#sec-r-univariate-helper}
...
```
1. A density function `d(x, log = FALSE)` with argument `x`.
2. A cumulative distribution function `p(q, lower.tail = TRUE, log.p = FALSE)`
with argument `q`. 
3. A quantile function `q(p, lower.tail = TRUE, log.p = FALSE)` with argument
`p`.
4. An indicator `s(x)` of whether argument `x` is in the distribution's support.

Arguments `log` and `log.p` are logical; if `TRUE`, probabilities are given on
the log-scale. Argument `lower.tail` is logical; if `TRUE`, probabilities are
$P(X \leq x)$ and $P(X > x)$ otherwise.


**TBD**: remove other R functions or document them?
...
```
1. The argument.
2. Logical; `log = true` specifies that the result should be returned on the
log-scale. Otherwise, it should be returned on the original scale.
...
**TBD**: note on how to get equal-spaced knots.
...
virtual double get_xi_upper(bool log = true) const = 0;        // <7>
virtual double get_xi_lower(bool log = true) const = 0;        // <8>
...
This is a subclass of `Region`, defined in @sec-region, specifically for
...
`UnivariateHelper` is a class which is intended for use with `UnivariateConstRegion`. It contains several operations needed from the base distribution $g$. These operations are specified as lambdas in the constructor.
...
## Log-Sum-Exp {#sec-cpp-api-logsumexp}
...
The following compute $\log\{ \sum_{i=1}^n \exp(x_i) \}$. The first form yields
a scalar based on a vector $x \in \mathbb{R}^n$. The second form applies the
computation to the rows of a matrix $X \in \mathbb{R}^{m \times n}$ to yield
...
The following are for addition on the log scale: $\log(e^x + e^y)$. The first
...
We make use of the Gumbel trick [e.g., @MaddisonTarlowMinka2014] to draw from a discrete distribution with probabilities $p_1, \ldots, p_k$. This approach allows probabilities to be specified on the log-scale without the need to exponentiate them or normalize so that they sum to one. The Gumbel trick generates a draw $x$ from the desired discrete distribution via
<!-- -->
$$
...
# Example: Bessel Count Distribution
