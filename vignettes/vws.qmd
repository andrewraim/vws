...
virtual bool is_bifurcatable() const = 0;                      // <6> // TBD: delete?
...
@sec-privacy-within-gibbs-r presents a version of the code where the Gibbs sampler is written in R and Rcpp modules are used to interact with VWS proposals written in C++, as in @sec-persistent-module. @sec-privacy-within-gibbs-cpp presents an alternative version of the code where the Gibbs sampler is written in C++ rather than R. The latter is substantially faster because it avoids the overhead of repeated calls between R and C++; however, it requires a much more C++ programming and cannot be debugged as readily as the first version in R.

## Gibbs Sampler in R {#sec-privacy-within-gibbs-r}

**TBD**: the time difference is more substantial when more time is spent in C++ and less is spent calling C++ from R. For example, when the whole Gibbs sampler is
coded in C++, we were seeing the tuned version take about 20% as long. See the results in @saevws-2025.
