...
Let us first consider a constant majorizer for $w$. Because this is our first example, the main source file for the sampler (`examples/vmf/vmf-v1.cpp`) is given below.
...
Note that the file `examples/vmf/texp.h` defines functions for the doubly truncated exponential distribution which are used above; this includes the density, CDF, and quantile function.

**TBD:** maybe just include the point above in the bulleted list.
...
\end{align*}
<!-- -->
where $g(x) = (a^2/4)^x e^{-a^2/4} \cdot \mathbb{1}_{\mathbb{N}}(x) / x!$
