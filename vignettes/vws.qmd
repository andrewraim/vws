...
Note that `FMMProposal` class, the `rejection` function, and other elements of the `vws` API are in the `vws` namespace. Details for the API are given in @sec-cpp-api. A complete working example is given in @sec-vmf-const-default. Once the `sample` function is successfully exported as an R function, we may invoke it as usual.
