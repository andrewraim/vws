...
1. We should have a separate section where we describe doing stuff: constructing the proposal (special cases and general case where we override abstract region), refining the proposal, ...
...
Readers looking for a quick start can go directly to the first example in @sec-vmf-const-default whose contents are described in the highest level detail. Code for the `vws` package is maintained in the GitHub repository at <https://github.com/andrewraim/vws>. 
:::
