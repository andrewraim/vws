...
- `Region` abstract class and how to override.
- We have already listed the operations that need to be implemented in `Region`.
- May be best to keep this section very short and refer to an example.
:::
...
- [x] Try to do even better and use a linear majorizer.
...
TBD: why are the first two samplers so slow to refine? They seemed much faster
...
```{r}
int_out = integrate(lower = z - 200, upper = z + 200, f = function(x) { 
...
TBD: explain interval.

```{r}
interval_lo = quantile(x, probs = 0.025)
interval_hi = quantile(x, probs = 0.975)
g + annotate("rect", xmin = interval_lo, xmax = hi, ymin = 0, ymax = Inf,
	alpha = 0.1, fill = "blue") +
...
- [ ] Now we use the linear majorizer. This is the place to give background on
  what that is.
- [ ] Give the second derivative and characterize when weight function is
  log-concave versus log-convex.
- [ ] We assume the transition point is not contained in the interior of any
  interval.
- [ ] Form the linear majorizer and minorizer for log-concave regions. For
  log-convex regions, we just swap the two.
- [ ] @VWS2024 suggest an optimization to find a $c$ to obtain the linear
  coefficients. This is based on computing a certain MGF. Show what that MGF is
  in this problem.
- [ ] The function `d_base` is just the density of $\text{N}(z, \lambda^2)$.
- [ ] The function `w_major` is
  $\overline{w}(x) = \exp\{ \overline{\beta}_{1j} + \overline{\beta}_{1j} x \}$.
- [ ] The `d` and `r` functions depend on $g_j$. Show that this is the
  distribution $\text{N}(z + \lambda^2 \overline{\beta}_{1j}, \lambda^2)$
  truncated to $(\alpha_{j-1}, \alpha_j]$.
- [x] The $\overline{\xi}_j$ and $\underline{\xi}_j$ functions are computed via
  similar integrals as the MGF.
- [x] We can state a general result for the integral $\int_a^b e^{tx} \phi(x) dx$ and
  reuse it in several places here.
- [ ] If possible, let's make a table to summarize the functions that are coded
  in the class.
- [ ] Link to the code for the class.
- [ ] Show some code to construct the sampler, refine it, and sample from it.
  Show the improved efficiency.
:::

Sections [-@sec-ln-norm-const] and [-@sec-ln-norm-const-cf] utilized a constant 
majorizer $\overline{w}_j = \max_{x \in \mathcal{D}_j} w(x)$ and minorizer $\underline{w}_j = \min_{x \in \mathcal{D}_j} w(x)$ for VWS sampling. Let us now consider a linear majorizer $\log \overline{w}_j(x) = \overline{\beta}_{0j} + \overline{\beta}_{1j} x$ and minorizer $\log \underline{w}_j(x) = \underline{\beta}_{0j} + \underline{\beta}_{1j} x$. This strategy is more involved to derive and implement, but often captures the weight function more effectively than a constant majorizer with fewer mixture components to yield more efficient proposals.

The following construction assumes that $\zeta(x) = \log w(x)$ is finite and concave on the interval $\mathcal{D}_j = (\alpha_{j-1}, \alpha_j]$. A majorizer is obtained from
<!-- -->
\begin{align*}
\log w(x) &\leq \log w(c) + (x - c) \nabla(c)
= \overline{\beta}_{j0} + \overline{\beta}_{j1} \cdot x
\end{align*}
<!-- -->
for $\overline{\beta}_{j0} = \log w_j(c) - c \cdot \zeta'(c)$, $\overline{\beta}_{j1} = \zeta'(c)$ and some $c \in \mathcal{D}_j$. In particular, let us consider the value of $c$ as
<!-- -->
\begin{align*}
...
\end{align*}
<!-- -->
where $M_j(s)$ is the moment generating function of random variable $T_j \sim g_j$.

TBD: in this case, the MGF is ...

Proceeding from the first derivative of $\zeta(x)$ in [@sec-ln-norm-const-cf], the second derivative is seen to be
<!-- -->
\begin{align*}
...
\end{align*}
<!-- -->
so that $\zeta(x)$ is concave when $x < \exp(\mu - \sigma^2 + 1)$ and convex when $x > \exp(\mu - \sigma^2 + 1)$.

TBD: To keep things simple, let's start with two regions $\mathcal{D}_1 = (0, \exp(\mu - \sigma^2 + 1)]$ and $\mathcal{D}_2 = (\exp(\mu - \sigma^2 + 1), \infty]$, so that for all regions considered thereafter, $w$ is either log-concave or log-convex on the entire region.

TBD: in log-concave regions we have majorizer and minorizer blah and blah.

TBD: in log-convex regions we switch them.
