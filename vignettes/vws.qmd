...
abstract: |
  ...
  VWS is an approach to construct proposals for rejection sampling which is
  explored in @vws-2026. The `vws` package provides an API to program the
...
This document is released to inform interested parties of ongoing research and to encourage discussion of work in progress. Any views expressed are those of the author and not of the U.S. Census Bureau.
...
The remainder of the vignette proceeds as follows. @sec-vws briefly reviews VWS sampling. @sec-overview gives an overview of the package, its major components, and important preliminaries for programming. @sec-api provides a detailed specification of the C++ API. @sec-vmf develops several VWS samplers to generate from the Von Mises Fisher (VMF) distribution following the application in @vws-2026. @sec-privacy develops samplers in a disclosure avoidance setting relevant to the U.S. Census Bureau, which is considered by @Direct-Sampling-DAS-2021, @DP-Simulation-2022, and @DAS-Methods-2025. @sec-bessel develops VWS samplers for the Bessel count distribution [@Devroye-2002]. In some applications, it may be desirable to have a persistent VWS proposal object in R that can gradually be refined and reused to draw samples; @sec-persistent presents several mechanisms to achieve this. @sec-privacy-within-gibbs extends the example from @sec-privacy to a Gibbs sampler and demonstrates VWS-within-Gibbs. The first variant uses persistent proposals, as in @sec-persistent, with a Gibbs sampler written in R and VWS implemented in C++. The second variant implements both in C++.
...
\end{align}
<!-- -->
The quantities $\rho_1, \ldots, \rho_N$ are the contributions of each region to $\rho_+$. The bound $\rho_+$ can be used to determine whether a VWS proposal will be viable for rejection sampling; if it is not much less than 1, rejections may be too frequent for practical use. The proposal may be refined by altering the partitioning or considering a different majorizer. Several specific choices of majorizer are considered by @vws-2026 and will be reviewed in the remainder of this section. @sec-vws-constant discusses the use of a constant majorizing function. A linear majorizing function is discussed in @sec-vws-linear. @sec-vws-knots reviews several knot selection methods.
...
\log w(x)
&\leq \log w(c) + (x - c) \nabla(c) \nonumber \\
&= \overline{\beta}_{0j} + \overline{\beta}_{1j} \cdot x,
\label{eqn:linear-concave-majorizer}
\end{align}
<!-- -->
so that
\begin{math}
\overline{w}_j(x) = \exp\{ \overline{\beta}_{0j} + \overline{\beta}_{1j} \cdot x \}
\end{math}
<!-- -->
with
<!-- -->
\begin{math}
\overline{\beta}_{0j} = \log w(c) - c \cdot \nabla(c),
\end{math}
<!-- -->
\begin{math}
\overline{\beta}_{1j} = \nabla(c),
...
&= \underline{\beta}_{0j} + \underline{\beta}_{1j} \cdot x,
...
<!-- -->
\begin{math}
\underline{w}_j(x) = \exp\{ \underline{\beta}_{0j} + \underline{\beta}_{1j} \cdot x \}
\end{math}
<!-- -->
with
<!-- -->
\begin{math}
\underline{\beta}_{0j} = \log w(\alpha_{j-1}) - \alpha_{j-1} \underline{\beta}_{1j}
\end{math}
<!-- -->
and
<!-- -->
\begin{math}
\underline{\beta}_{1j} = \{\log w(\alpha_j) - \log w(\alpha_{j-1}) \} / \{ \alpha_j - \alpha_{j-1} \}.
\end{math}
\remarkqed
:::
...
The following remark describes one possible choice for the expansion point $c$ which will be utilized in the examples in Sections [-@sec-vmf-linear], [-@sec-privacy-linear], and [-@sec-bessel-linear].
...
<!-- -->
\begin{math}
\overline{w}_j(x) = \exp\{ \underline{\beta}_{0j} + \underline{\beta}_{1j} \cdot x \},
\end{math}
<!-- -->
\begin{math}
\underline{\beta}_{0j} = \log w(c) - c \cdot \nabla(c),
\end{math}
<!-- -->
\begin{math}
\underline{\beta}_{1j} = \nabla(c),
...
## Self Tuning {#sec-self-tune}

VWS may be used to generate from conditional distributions in a Gibbs sampler; however, the amount of computation can become arduous if new proposals are constructed in each step of the Gibbs sampler. In this setting, @saevws-2025 consider a persistent VWS proposal that is gradually tuned to track a particular conditional distribution in the Gibbs sampler as the MCMC chain evolves. If the target does not change much as the chain approaches the posterior distribution, the amount of tuning needed to maintain a rejection rate which is sufficiently below 1 should also diminish.

Two types of updates are considered when a proposed draw $x$ is rejected. If bound $\rho_+$ is larger than a given threshold $\epsilon_1 \in (0,1)$, $x$ is used to refine the current set of regions. The region $\mathscr{D}_j$ containing $x$ is selected for bifurcation, but the specific way that $x$ is used to bifurcate depends on the nature of the regions. For example, there are a number of possibilities to bifurcate a multivariate region at point $x$. On the other hand, if $\rho_+$ is smaller than $\epsilon_1$, we check for regions with small contributions of $\rho_\ell$ to $\rho_+$. In particular regions with $\rho_\ell$ smaller than a given threshold $\epsilon_2 \in (0,1)$ are coarsened (i.e., merged with a neighboring region) if doing so does not increase $\rho_+$ beyond $\epsilon_1$. This process is repeated until no further merges can be completed. 

To emphasize, $\epsilon_1$ represents a threshold for a sufficiently small rejection rate while $\epsilon_2$ represents a tolerance for low contribution. In practice, a sufficiently small rejection rate need not be far from 1 if rejections are computationally inexpensive; a value such as $\epsilon_1 = 0.85$ may yield good performance.

Algorithm \ref{alg:self-tune} represents rejection sampling with self-tuning described in the manner above. It makes use of Algorithm \ref{alg:merge} to handle merges. Note that only certain pairs of regions are eligible to be merged, and eligibility is determined by the particular type of region. For example, the regions implemented in @sec-api-realconstregion and @sec-api-intconstregion are based on univariate intervals; here, only adjacent intervals having a common endpoint can be merged together. There may be multiple merges possible with a given partition, where different choices on line  \ref{step:merge} of Algorithm \ref{alg:merge} yield different configurations after the loop on line \ref{step:merge-loop} of Algorithm \ref{alg:self-tune}. For the sake of simplicity, the current implementation performs the first eligible merge in each traversal of the loop rather than considering all possible merges.

@saevws-2025 present several examples where tuning can effectively be limited to an initial part of the MCMC chain and discontinued for the remainder of the chain. Here the number of rejections per iteration can be monitored to ensure that proposals remain adequate as the chain progresses. If rejections increase dramatically after tuning is halted, a longer tuning period may be needed.

\begin{algorithm}
\caption{Generate a single draw from $f$ using self-tuned VWS with tolerances $\epsilon_1$ and $\epsilon_2$.}
\label{alg:self-tune}
\begin{algorithmic}[1]
\Function{RejectionTune}{$h$ by reference, $\epsilon_1$, $\epsilon_2$}
\Do
\State Draw $u$ from $\text{Uniform}(0,1)$ and $x$ from $h$.
\State $\text{Accept} \gets \ind\{ u \leq f_0(x) / h_0(x) \}$ where $f_0(x) = w(x) g(x)$ and $h_0(x) = \overline{w}(x) g(x)$.
\State Let $\rho_{+}$ be the current value of bound \eqref{eqn:bound}.
\If{$\text{Accept} = 0$ and $\rho_{+} < \epsilon_1$}
\Do
\label{step:merge-loop}
\State $\text{Repeat} \leftarrow \text{MergeOnce}(h, \epsilon_1, \epsilon_2)$.
\doWhile{$\text{Repeat} = 1$}
\ElsIf{$\text{Accept} = 0$ and $\rho_{+} \geq \epsilon_1$}
\State Refine $h$ at $x$.
\EndIf
\doWhile{$\text{Accept} = 0$}
\State \Return $x$ and $h$.
\EndFunction
\end{algorithmic}
\end{algorithm}

\begin{algorithm}
\caption{Attempt to merge a single pair of regions in $h$. Return $1$ if a merge was completed and $0$ otherwise.}
\label{alg:merge}
\begin{algorithmic}[1]
\Function{MergeOnce}{$h$ by reference, $\epsilon_1$, $\epsilon_2$}
\State Let $\mathscr{D}_1, \ldots, \mathscr{D}_N$ be the regions corresponding to current mixture components of $h$.
\State Let $\rho_{+}$ be the current value of bound \eqref{eqn:bound} and $\rho_1, \ldots, \rho_N$ be contribution of each region.
\For{$j = 1, \ldots, N$}
\If{$\rho_j < \epsilon_2$}
\State Let $\ell$ ($\neq j$) be the index of a region that can be merged with $\mathscr{D}_j$.
\label{step:merge}
\State Let $\tilde{h}$ be equal to $h$ with regions $\mathscr{D}_j$ and $\mathscr{D}_\ell$ merged.
\State Let $\tilde{\rho}_+$ be the bound \eqref{eqn:bound} for $\tilde{h}$.
\If{$\tilde{\rho}_+ < \epsilon_1$}
\State Modify $h$ to $\tilde{h}$.
\State \Return $1$.
\EndIf
\EndIf
\EndFor
...
The `vws` package aims to support the methodology which was described in the previous section. The present section will describe tools in the package which can be used to formulate a problem, construct a proposal, and generate samples. Some useful preliminaries are also presented.

## Design {#sec-overview-design}

Use of the `vws` package centers on two classes. The `fmm_proposal` class represents a finite mixture of the form \eqref{eqn:fmm-proposal} and encapsulates the operations needed for rejection sampling. The `Region` class represents region $\mathscr{D}_j$ and the operations required for VWS. `Region` itself is an abstract base class which defines an interface needed by the framework. All problem-specific logic is coded within a subclass of `Region`: either by using one of the provided subclasses or by coding a new subclass. The subclasses `real_const_region` and `int_const_region` provide implementations of the constant majorizer from @sec-vws-constant, for continuous and discrete univariate distributions, respectively. The linear majorizer described in @sec-vws-linear requires more customization than the constant majorizer; here, users should create a subclass of `Region`. An `fmm_proposal` object is created from $N \geq 1$ `Region` objects that represent the partition $\mathscr{D}_1, \ldots, \mathscr{D}_N$ of $\Omega$.

The `rejection` function takes an object `h` of class `fmm_proposal` and carries out rejection sampling to obtain $n$ draws. The `rejection` function returns a vector of accepted draws along with information about the number of rejections. @fig-software-design displays a diagram of the high-level design just described.

![Design of `vws` package.](software-design.pdf){#fig-software-design}

## Typical Workflow {#sec-overview-workflow}
...
5. Construct an `fmm_proposal` based on a single region `supp`. Note that it is a template class that needs two template arguments: the first (`double`) indicates the data type of the distribution and the second (`MyRegion`) indicates the type of region it will contain.
...
Note that the `fmm_proposal` class, the `rejection` function, and other elements of the `vws` API are accessed in the `vws` namespace. Details for the API are given in @sec-api. Once the `sample` function is successfully exported as an R function, we may invoke it as usual. Suppose the code in the previous display is saved is in a file named `sample.cpp`.
...
A complete VWS example is given in @sec-vmf-const-default; this demonstrates the use of `real_const_region` to implement a sampler with a minimal amount of coding. @sec-vmf-const-custom is an improvement which requires more coding; here we provide functions to determine values of the constant majorizer and minorizer analytically. Finally, @sec-vmf-linear demonstrates how to subclass `Region` to implement a linear majorizer, which requires more intricate coding. Subsequent examples are given in @sec-privacy and @sec-bessel, but with less explanation.
:::

## Lambdas {#sec-overview-lambdas}
...
## Computations on the Log-Scale {#sec-logscale}
...
## Typedefs and Aliases {#sec-api-typedefs}
...
The initial release of the `vws` package used a "camelcase" convention for class names, which has since changed to lowercase with underscores. The following aliases are defined for backward compatibility.
...
The class `fmm_proposal` represents a VWS proposal in the form of \eqref{eqn:fmm-proposal}.

### Class Definition {.unlisted}

`fmm_proposal` has two template arguments: `T` is the data type for the underlying distribution (e.g., `T = double` for a real-valued univariate random variable) and `R` is the type of region which the proposal will be based upon.
...
4. Get a vector of zero-based indices corresponding to regions which are mergeable to the region with index `i`. This vector does not include `i` itself and may be empty.
5. Get the mixing proportions $\pi_1, \ldots, \pi_N$.
6. Get the contributions $\rho_1, \ldots, \rho_N$ to bound \eqref{eqn:bound} corresponding to the $N$ regions.
7. Get the overall rejection bound \eqref{eqn:bound}.
8. Get the normalizing constant $\psi_N$
9. Get the number of regions $N$.
...
The following methods provide iterators to the set of regions in the proposal: const for read-only access and non-const for read and write access. Using them can be more efficient than the accessors in @sec-api-proposal because they do not make a copy of the data.
...
Two functions are provided to refine the proposal from $\mathscr{D}_1, \ldots, \mathscr{D}_N$ into a finer partition. Both variants return a vector $\rho_+^{(0)}, \ldots, \rho_+^{(N_0)}$ which represents values of the bound \eqref{eqn:bound} at each refinement step; $N_0$ is the number of steps taken, $\rho_+^{(j)}$ represents the value at refinement $j = 1, \ldots, N_0$, and $\rho_+^{(0)}$ represents the initial value. The first variant partitions at a given vector of knots.
...
The second variant uses the rule of thumb for sequential knot selection from @vws-2026. Refining will halt when \eqref{eqn:bound} reduces below `tol`; which is possible when `tol` is positive; otherwise, refining halts after `N` steps.
...
1. Maximum number of refinements to make.
...
The two `refine` functions depend on methods `is_bifurcatable` and `bifurcate` of the region. These determine how to bifurcate the region and if it is eligible to be bifurcated. For a subclass `MyRegion` of `Region` whose elements are of type `T`, these methods should be defined as follows.
...
```
1. Indicator of whether the current region can be bifurcated.
2. Bifurcate the current region, but without specifying a point of reference as an argument.
3. Bifurcate the current region using `x` as a point of reference.
...
### Coarsening the Proposal {.unlisted}

The `merge` function merges the pair of regions with indices `i` and `j` so that number of regions $N$ in the proposal is reduced by one. The return value is the updated value of bound \eqref{eqn:bound} after merging; this value is returned on the log-scale if argument `log = true`.

```{cpp}
double merge(unsigned int i, unsigned int j, bool log = true)
```

The `merge` function depends on methods `is_mergeable` and `merge` of the region. These determine how to merge two regions and whether they are eligible to merged. For a subclass `MyRegion` of `Region`, these methods should be defined as follows.

```
bool is_mergeable(const MyRegion& x) const; // <1>
MyRegion merge(const MyRegion& x) const;    // <2>
```
1. Indicator of whether the region in argument `x` can be merged with this region.
2. Construct a region by merging the region in argument `x` with this region.
...
: Data frame returned by summary method of `fmm_proposal`. {#tbl-summary tbl-colwidths="\[30,70\]"}
...
6. The quantity $\overline{\xi}_j$ for this region.
7. The quantity $\underline{\xi}_j$ for this region.
8. Compute the portion $(\overline{\xi}_j - \underline{\xi}_j) / \overline{\xi}_j$ of bound \eqref{eqn:bound} specific to this region.
...
3. Indicator of whether region can be bifurcated.
4. Indicator of whether region can be merged with region `x`. For example, if regions are intervals, this condition may be equivalent to testing whether the current regions is adjacent to `x`.
5. Return a region on the singleton set $(x,x]$ with the information in the current object.
6. Comparison operator; used to order regions of this type.
7. Equality operator; check whether two regions of this type are equal.
8. Assignment operator; assign information from `x` to this object.

## Region on Real-Valued Support with Constant Majorizer {#sec-api-realconstregion}

`real_const_region` is a subclass of `Region`, defined in @sec-api-region, specifically for univariate problems with continuous support where $\overline{w}(x) = \sum_{j=1}^N \overline{w}_j \ind\{x \in \mathscr{D}_j\}$ is constructed from constants $\overline{w}_1, \ldots, \overline{w}_N$. It assumes a constant minorizer $\underline{w}(x) = \sum_{j=1}^N \underline{w}_j \ind\{x \in \mathscr{D}_j\}$ is constructed from constants $\underline{w}_1, \ldots, \underline{w}_N$. The $\overline{w}_j$ and $\underline{w}_j$ are obtained using numerical optimization by default; however, user-supplied methods can be supplied to identify these values when it is possible.

### Constructors {.unlisted #sec-api-realconstregion-constructors}

`real_const_region` has the following constructors. The first creates a region based on the interval $(a,b]$; the second creates a region based on the singleton set $\{a\}$, which is intended primarily for internal use.
...
7. A function of type `midpoint` to compute the midpoint of region's interval.

The default optimizers, `maxopt_default` and `minopt_default`, use a hybrid numerical optimization method in @sec-api-opt to optimize `w`.
...
The `merge` method produces a region consisting of the current region and the argument `x` merged together.

```{cpp}
real_const_region merge(const real_const_region& x) const;
```

The `is_mergeable` function is an indicator of whether the current region can be merged with the argument `x`. This equates to `true` if and only if the two regions are adjacent intervals whose endpoints meet at a common value (on either side).

```{cpp}
bool is_mergeable(const real_const_region& x) const;
```

Several functions are provided to update an existing region.
...
void init();                                       // <6>
```
1. Update the weight function.
2. Update the univariate helper which represents the base distribution.
3. Update the maximization routine for the majorizer.
4. Update the minimization routine for the minorizer.
5. Update the function to find the midpoint.
6. Recompute saved quantities such as the majorization and minorization constants needed for rejection sampling.

::: {.callout-important title="init"}
The `init` method should be applied after using the set methods above, before the region is used in any other capacity. More than one set method may be used before calling `init`.
:::
...
The class `int_const_region` is a subclass of `real_const_region` that is specialized to integer-valued distributions.
...
`int_const_region` has the following constructors.
...
The arguments here are analogous to those in @sec-api-realconstregion-constructors. Note that `a` and `b` need not be integers here.

### Methods {.unlisted}

The following methods are specialized from `real_const_region`.
...
The class `univariate_helper` is intended for use with `real_const_region` and `int_const_region`. It encapsulates several operations needed from the base distribution $g$. These operations are specified as lambdas in the constructor.
...
The following methods on `univariate_helper` utilize the lambdas specified above.
...
### Without Tuning {#sec-api-rejection-basic}

The following functions perform rejection sampling using a VWS proposal described in @sec-vws.
...
```
1. An `fmm_proposal` object to use as the proposal.
2. The number of desired draws.
3. Additional arguments for rejection sampling. Default values are assumed in the second form.

Template arguments `T` and `R` correspond to the given proposal `h` and are described in @sec-api-proposal.

### With Tuning {#sec-api-rejection-tune}

The following functions perform rejection sampling with tuning as described in @sec-self-tune.
...
### Additional Arguments {#sec-api-rejection-args}

Additional arguments to `rejection` and `rejection_tune` are provided via the following struct.
...
5. Tolerance $\epsilon_1$ in Algorithm \ref{alg:self-tune} used to determine whether rejection rate is sufficiently below 1. This is used only in `rejection_tune`.
6. Tolerance $\epsilon_2$ in Algorithm \ref{alg:self-tune} used to determine whether a region's contribution is low enough to be merged with another region.
This is used only in `rejection_tune`.
7. Determines whether to save additional metrics. If `true`, non-essential metrics in `rejection_result` will be populated. Otherwise, only `draws` will be populated with the generated variates.
8. Constructor that takes no arguments and initializes to default values.
9. Convert an `Rcpp::List` to a `rejection_args` struct.
10. Return a `Rcpp::List` from a `rejection_args` struct.
...
: Value of `action` used in `rejection_args.action` and its effect in `rejection`. {#tbl-action tbl-colwidths="\[35,65\]"}

### Results {#sec-api-rejection-results}

The return value of `rejection` and `rejection_tune` is a struct of the following type.
...
3. Vector of tuning counts; the $i$th element represents number of tuning adjustments made before accepting the $i$th draw.
4. Vector of region counts; the $i$th element represents number of regions in the proposal when accepting the $i$th draw.
5. Return a `Rcpp::List` from a `rejection_args` struct.

The `tunes`, `regions`, and `log_bounds` fields are only relevant to `rejection_tune`; they are left empty in `rejection`. Furthermore, all fields aside from `draws` are left empty when `metrics = false` in `rejection_args`.

If the maximum number of rejections are reached and `rejection_args` specifies `action = fntl::error_action::STOP`, the length of the vectors `draws` and `rejects` will be less than $n$.
...
As mentioned in @sec-logscale, calculations in the package are carried out on the log-scale, where possible, to avoid issues from floating point numbers with very small or very large magnitudes. Users may want to follow this convention when implementing their own sampling problems. Several included functions help to avoid explicit exponentiation.

The following computes the scalar $f(x) = \log\{ \sum_{i=1}^n \exp(x_i) \}$ from a vector $x \in \mathbb{R}^n$ using the method described in [this StackExchange post](https://stats.stackexchange.com/a/381937).
...
<!-- -->
\begin{displaymath}
\log(e^x + e^y) = t + \log\{1 + \exp[s - t]\},
\quad s = \min(x,y), \quad t = \max(x,y).
\end{displaymath}
<!-- -->
The first form takes scalar arguments $x$ and $y$. The second and third forms take $x, y \in \mathbb{R}^n$ and produce a vector with elements $\log(e^{x_i} + e^{y_i})$, $i = 1, \ldots, n$.
...
\end{displaymath}
<!-- -->
When $x$ is smaller than $y$, the results is assumed to be `nan`. The first form takes scalar arguments $x$ and $y$. The second and third forms take $x, y \in \mathbb{R}^n$ and produce a vector with elements $\log(e^{x_i} - e^{y_i})$, $i = 1, \ldots, n$. 
...
## Formatted Print with Timestamp {#sec-api-logger}

The following function writes a formatted message to the console preceded by a timestamp. 

```{cpp}
void logger(const char* fmt, ...);
```

The arguments should be used in the same way as the C function `printf`. The timestamp is formatted as `"%Y-%m-%d %H:%M:%S"` and is in the local time zone of the machine. A dash `"-"` is displayed between the timestamp and the formatted message with one space of padding on each side. A reference for formatted messages can be found in R under the manual page `?sprintf`; for dates and times, see the manual page `?strptime`.

# Example: Von Mises Fisher Distribution {#sec-vmf}

Let us consider generating from the von Mises Fisher (VMF) distribution as in @vws-2026. VMF is useful in modeling directional data whose support is the sphere $\mathbb{S}^{d-1} = \{ \vec{v} \in \mathbb{R}^d : \vec{v}^\top \vec{v} = 1 \}$ [e.g., @Mardia-Jupp-1999]. A random variable $\vec{V}$ with distribution $\text{VMF}_d(\vec{\mu}, \kappa)$ has density
<!-- -->
\begin{align*}
...
::: {#rem-vmf-caveat}
A caveat of this decomposition is that, in the $d < 3$ case, $w(x) \rightarrow \infty$ as $x$ approaches $\pm 1$. One way to avoid this is by truncating the support to $(\alpha_0, \alpha_N] = (-1 + \epsilon, 1 - \epsilon]$ for a small $\epsilon > 0$. Rejection sampling can proceed using the truncated support if the exclusion of $(-1, -1 + \epsilon] \cup (1 - \epsilon, 1]$ is known to have a negligible impact on the result. Otherwise, @vws-2026 mention another strategy where the support is initially truncated and gradually expanded as rejections are encountered. In this document, we assume $(\alpha_0, \alpha_N] = (-1, 1]$ for $d > 3$ and a fixed truncation $(\alpha_0, \alpha_N] = (-1 + \epsilon, 1 - \epsilon]$ for $d < 3$.
:::
...
9. Construct a `real_const_region` which contains all problem-specific logic of the sampler. We construct one initial region which contains the entire support $(-1,1]$.
10. Construct an `fmm_proposal` based on our initial region `supp`. The first template argument specifies that the data type of the support is `double`; the second specifies that regions (which include the logic of the sampler) are of type `real_const_region`.
...
The name of the function `r_vmf_pre_v1` reflects that this is our first version of the sampler for target \eqref{eqn:vmf-target}, which is a precursor to transformation \eqref{eqn:vmf-tx} to obtain a VMF random variable. The following R snippet builds the C++ code and invokes the sampling function.
...
We noted in @sec-vmf-const-custom that $w$ is log-convex when $d < 3$, log-concave when $d > 3$, and a constant otherwise. Therefore, we can majorize $w$ with exponentiated linear functions of the form $\overline{w}_j(x) = \exp\{ \overline{\beta}_{0j} + \overline{\beta}_{1j} x \}$. This yields the expression
<!-- -->
\begin{align*}
\overline{\xi}_j
= \int_{\alpha_{j-1}}^{\alpha_j} \overline{w}_j(x) g(x) dx
= \frac{\kappa \exp\{ \overline{\beta}_{0j} \}}{(\kappa + \overline{\beta}_{1j}) (e^{\kappa \alpha_N} - e^{\kappa \alpha_0})} \left\{
\exp\{(\kappa + \overline{\beta}_{1j}) \alpha_j\} - \exp\{(\kappa + \overline{\beta}_{1j}) \alpha_{j-1} \}
\right\}.
\end{align*}
<!-- -->
...
&= \overline{w}_j(x) g(x) \ind\{ x \in \mathscr{D}_j \} / \overline{\xi}_j \\
&= \frac{
(\kappa + \overline{\beta}_{1j}) \exp\{(\kappa + \overline{\beta}_{1j}) x\}
}{
\exp\{(\kappa + \overline{\beta}_{1j}) \alpha_j\} - \exp\{(\kappa + \overline{\beta}_{1j}) \alpha_{j-1} \}
}
...
\end{align*}
<!-- -->
the density of $\text{Exp}_{(\alpha_{j-1}, \alpha_j]}(\kappa + \overline{\beta}_{1j})$. @rem-expansion-point is used to select the expansion point $c$ to determine coefficients for the majorizer in the log-concave case, with
<!-- -->
\begin{align*}
...
# Example: Privacy Protected Data {#sec-privacy}

Suppose an official statistics agency collects sensitive data from respondents in a survey or census. The agency then seeks to disseminate a tabulation to data users while protecting the privacy of the respondents. The agency can consider adding random noise $\gamma$ to the tabulation $Y$ and releasing $Z = Y + \gamma$. This setting is the basis of a modeling scenario considered by @Direct-Sampling-DAS-2021, @DP-Simulation-2022, and @DAS-Methods-2025 where the distribution for $\gamma$ has been selected by the agency and the data have been disseminated; the objective in these works is to carry out inference on the unobserved $Y$ given observed $Z = z$. The field of differential privacy studies the design of noise mechanisms (including distributions of $\gamma$ in the present setting) to satisfy mathematical criteria for privacy [e.g., @Dwork-Roth-2014]. @Abowd-EtAl-2022 describe work by the U.S. Census Bureau to implement differential privacy in the release of data from the decennial census.

Our present motivation is to focus on a specific target that arises in analysis of the released $z$. @sec-privacy-within-gibbs considers a data analysis situation with $n$ subjects and a regression underlying $y$. To reduce the amount of computation under repeated use within a Gibbs sampler, we will consider proposals that persist over the MCMC chain and get updated with the current state.
...
\end{math}
<!-- -->
from the Lognormal component---dropping some of the terms in its normalizing constant---and base distribution whose density $g$ is $\text{N}(z, \lambda^2)$. The conditional $[\vec{y} \mid \sigma^2, \vec{\mu}, \vec{z}]$ in \eqref{eqn:privacy} would be encountered in a Gibbs sampler for the posterior distribution $[\vec{y}, \vec{\mu}, \sigma^2 \mid \vec{z}]$, based on an observed sample $\vec{z} = (z_1, \ldots, z_n)$ with augmented data $\vec{y} = (y_1, \ldots, y_n)$. The remaining conditionals $[\vec{\mu} \mid \sigma^2, \vec{y}, \vec{z}]$ and $[\sigma^2 \mid \vec{\mu}, \vec{y}, \vec{z}]$ may be straightforward to generate if a convenient prior distribution is selected; therefore, we will focus on \eqref{eqn:privacy}.
...
We will consider three variations of a VWS sampler, progressing from easier-to-implement to more computationally efficient. @sec-privacy-const-default considers a constant majorizer where the constant for each region is obtained by numerical optimization. @sec-privacy-const-custom replaces numerical optimization with code for a closed-form solution. @sec-privacy-linear makes use of a linear majorizer which subclasses the abstract `Region` class. All codes for this example are in the folder `examples/privacy`. The function `d_target` defined in `examples/privacy/functions.R` computes the target density.

## Constant Majorizer with Numerical Optimization {#sec-privacy-const-default}
...
The base distribution's density, CDF, and quantile function are coded as lambdas and packaged into a `univariate_helper` object. Here we can make use of the implementations of the normal distribution in R's API.
...
@fig-privacy-const-refine plots the bound for the rejection probability during the refinement process, which is captured in the variable `out$lbdd`. @fig-privacy-const-draws plots the empirical distribution of the draws and compares them to the density. It displays an interval based on the $0.025$ and $0.975$ quantiles of the distribution $[Y \mid Z = z]$ approximated from the empirical quantiles of the draws. The value of the observed $z$ and the latent $y$ are also highlighted for reference.
...
## Constant Majorizer with Custom Optimization {#sec-privacy-const-custom}

The log-weight function can be both maximized and minimized in closed form. Coding it explicitly reduces computational overhead and avoids convergence issues from numerical optimization. This section will demonstrate how to override the default `optimize` method of `real_const_region`.
...
\end{align*}
<!-- -->
Therefore, on a region $\mathscr{D}_j = (\alpha_{j-1}, \alpha_j]$ where both endpoints are smaller than $y^*$, the maximum of $\log w(y)$ is $\log w(\alpha_j)$ and the minimum is $\log w(\alpha_{j-1})$. On the other hand, for a region where both endpoints are larger than $y^*$, the maximum of $\log w(y)$ is $\log w(\alpha_{j-1})$ and the minimum is $\log w(\alpha_j)$. @fig-privacy-weight displays a plot of $\log w(y)$ with our selected $\mu$ and $\sigma^2$ values.
...
We can construct the proposal using a single `real_const_region` that represents the support; the `maxopt` and `minopt` arguments are specified here.
...
## Linear Majorizer {#sec-privacy-linear}
...
\end{align*}
<!-- -->
which has root $y_0 = \exp\{ \mu - \sigma^2 + 1 \}$. The weight function $w$ is log-concave for $y < y_0$ and log-convex for $y > y_0$. This is plotted in @fig-privacy-weight-convexity. We will assume that there are two initial regions $\mathscr{D}_1 = (0, y_0]$ and $\mathscr{D}_2 = (y_0, \infty]$ so that all partitions considered thereafter consist of regions on which $\log w(y)$ is entirely concave or convex.
...
As in @sec-vmf-linear, we can majorize $w$ with exponentiated linear functions of the form $\overline{w}_j(y) = \exp\{ \overline{\beta}_{0j} + \overline{\beta}_{1j} y \}$. Here, we also assume a minorizer of the form $\underline{w}_j(y) = \exp\{ \underline{\beta}_{0j} + \underline{\beta}_{1j} y \}$. Before proceeding, the following remark gives an integral that will be used several times.
...
%
&= \exp\{ \overline{\beta}_{0j} \} \int_{\alpha_{j-1}}^{\alpha_j} 
\exp\{ \overline{\beta}_{1j} y \}
...
&= \frac{
\frac{1}{\lambda \sqrt{2\pi}}
\exp\left\{ -\frac{1}{2 \lambda^2} (y - z - \lambda^2 \overline{\beta}_{1j})^2 \right\}
}{
\Prob(\alpha_{j-1} < \overline{T} \leq \alpha_j)
}
...
\end{align*}
<!-- -->
Adapting the code from @sec-vmf-linear to the present problem, we implement `LinearVWSRegion` as a subclass of the abstract `Region` base class (@sec-api-region). See the file `examples/privacy/LinearVWSRegion.h`. The code in `examples/privacy/ln-norm-v3.cpp` instantiates a proposal from this region type and proceeds with rejection sampling. This code is displayed below.

``` {.cpp include="examples/privacy/ln-norm-v3.cpp" code-line-numbers="true"}
```
...
@fig-privacy-linear-draws plots the empirical distribution of the draws and compares them to the density. @fig-privacy-refine plots the bound for the rejection probability for this sampler after each step of refining, along with those from the previous two versions. A substantial improvement in efficiency is seem here; fewer regions are needed to achieve a small rejection probability. Although versions 1 and 2 of the sampler are based on the same proposal, the changes to their bounds are seen to differ due to randomness in knot selection.
...
Where Sections [-@sec-vmf] and [-@sec-privacy] considered continuous distributions, we now generate from a discrete target. The Bessel distribution [@Yuan-Kalbfleisch-2000] has density
<!-- -->
\begin{align*}
...
Three variations of VWS samplers are considered in subsections. @sec-bessel-const-default uses a constant majorizer obtained by numerical optimization. @sec-bessel-const-custom replaces numerical optimization with a closed-form solution. @sec-bessel-linear uses a linear majorizer and subclasses the abstract `Region` class. All codes for this example are in the folder `examples/bessel`. The function `d_bessel` defined in `examples/bessel/bessel.R` computes the target density.
...
The base distribution’s density, CDF, and quantile function may be supplied as a `univariate_helper` object using Poisson functions in the R API.
...
We can construct the proposal using a single `int_const_region` that represents the support; the `maxopt` and `minopt` arguments are specified here. Also note that we have specified a lower endpoint $\alpha_0$ below zero to ensure that the support $(\alpha_0, \alpha_N]$ contains zero.
...
Our decomposition of $f$ into the given $w$ and $g$ lends itself well to a linear majorizer because the function $\log w(x) = -\log \Gamma(x + \nu + 1)$ is log-concave. In fact, a theorem attributed to @Bohr-Mollerup-1922 states that the function $q(x) = \Gamma(x)$ is uniquely characterized as the only function on $(0, \infty)$ which is log-convex with $q(1) = 1$ and $q(x+1) = x q(x)$. The following remark is also useful to formulate the linear majorizer.

::: {#rem-incgamma}
By rearranging equation 6.5.13 of @Abramowitz-Stegun-1972, the cdf of $T \sim \text{Poisson}(\lambda)$ can be written as
<!-- -->
\begin{displaymath}
F(x \mid \lambda)
...
%
&= \exp\{ \overline{\beta}_{0j} \}
\sum_{k \in (\alpha_{j-1}, \alpha_j]}
\exp\{ \overline{\beta}_{1j} x \}
...
%\sum_{k \in (\alpha_{j-1}, \alpha_j]}
%\frac{1}{x!}
%\left[ e^{ \frac{\lambda^2}{4}\overline{\beta}_{1j} } \right]^x \\
%
&= \exp\left(
\overline{\beta}_{0j} - \frac{\lambda^2}{4}
+ \frac{\lambda^2}{4} e^{ \overline{\beta}_{1j} } \right)
\Prob(\alpha_{j-1} < \overline{T} \leq \alpha_j),
\end{align*}
<!-- -->
where $\overline{T} \sim \text{Poisson}(e^{ \overline{\beta}_{1j}} \lambda^2 / 4)$, and similarly,
<!-- -->
\begin{align*}
\underline{\xi}_j
%
...
\end{align*}
<!-- -->
where $\underline{T} \sim \text{Poisson}(e^{ \underline{\beta}_{1j}} \lambda^2 / 4)$. The proposal $h$ is a finite mixture $h(x) = \sum_{j=1}^N \pi_j g_j(x)$ with
<!-- -->
\begin{align*}
g_j(x)
&= \overline{w}_j(x) g(x) \ind\{ x \in \mathscr{D}_j \} / \overline{\xi}_j \\
...
&=
\frac{1}{x!}
\left(\frac{\lambda^2}{4} e^{ \overline{\beta}_{1j} } \right)^x
\exp\left( -\frac{\lambda^2}{4} e^{ \overline{\beta}_{1j} } \right)
\frac{1}{
\Prob(\alpha_{j-1} < \overline{T} \leq \alpha_j)
}
...
\end{align*}
<!-- -->
which is the density of $\text{Poisson}(\frac{\lambda^2}{4} e^{ \overline{\beta}_{1j}})$ truncated to the interval $(\alpha_{j-1}, \alpha_j]$. @rem-expansion-point is used to select the expansion point $c$ to determine coefficients for the majorizer in the log-concave case, with
<!-- -->
\begin{align*}
M_j(s)
...
# Persistent Proposals in R {#sec-persistent}

The samplers developed in previous sections each expose a function in R which can be used to generate samples. A new proposal is constructed each time the function is called and it exists only for the duration of the call. For the sake of efficiency or to maintain the state within a proposal, it may be desirable to have a persistent proposal that can be gradually reused and refined. This section demonstrates several Rcpp mechanisms to enable this workflow: first using external pointers then using using modules. The latter is based on object-oriented programming but is recommended over the former as it is less susceptible to crashing R. We demonstrate the two approaches by revisiting the example from @sec-vmf-const-default. For more information on external pointers and modules, refer to the vignette "Exposing C++ functions and classes with Rcpp modules" in the `Rcpp` package [@Rcpp] and section "External pointers and weak references" of the guide "Writing R Extensions" [@R-ext].

The approach from @sec-persistent-module using Rcpp modules is used in @sec-privacy-within-gibbs to generate from an unfamiliar conditional within a Gibbs sampler.

## External Pointers {#sec-persistent-xptr}

An Rcpp external pointer (`XPtr`) can be used to construct objects in C++ and return them to R so that they persist and can be accessed in subsequent calls to C++. This is especially useful with complicated data structures that are not easy to directly represent as R objects. A VWS proposal is an example of such as structure. The full C++ code is given in `examples/vmf/vmf-v1-xptr.cpp`. Here we describe the components of that file.
...
The following function creates an `XPtr` to a proposal. There are several notable changes from the implementation in @sec-vmf-const-default.
...
```
1. The function returns an `XPtr` to a proposal rather than the result of sample generation.
2. Capture by value with the lambdas here using `[=]`. Variables `kappa` and `d` may not be available in the environment when evaluating the lambdas later. Capture by value ensures that variables can still be accessed by the lambdas.
3. Create a proposal object on the heap using `new` and wrap it in an `XPtr` to be returned to R.
4. The function `set_tag` is a utility we will define; more explanation is provided below.

The following functions make use of the proposal `XPtr`. The `refine` function
modifies the proposal by applying up to `N` refinement steps of Algorithm \ref{alg:refine} until toleration `tol` is achieved. The `draw` function draws
a sample of size `n`.
...
```
1. The first argument is expected to be the `XPtr` obtained from `vmf_pre_v1_xptr`.
2. The function `assert_tag` is a utility we will define; more explanation is provided below.
3. The `refine` member function is called on the object pointed to by `h`; this modifies it in place.
...
For demonstration purposes, let us define a C++ function that returns an `XPtr` to a vector (i.e., something which is not a proposal object). When an `XPtr` to a proposal is expected but an `XPtr` to a vector is encountered, ensure that an appropriate exception is thrown.
...
::: {#rem-capture-value}
In the current implementation of `vws`, lambdas `w`, `df`, `pf`, and `qf` are copied to all regions used in the sampler. The overhead created by copying will likely not be significant in most instances, but could be an issue when very large objects are captured by value. In such cases, users may consider using pointers rather than values to capture these objects.
\remarkqed
:::

## Modules {#sec-persistent-module}

The external pointer approach in @sec-persistent-xptr lets us operate on a proposal object from R through C++ functions. We can take this one step further with object-oriented programming so that the proposal appears as a class in R that includes its methods. The complete C++ code for this example is in `examples/vmf/vmf-v1-module.cpp`.
...
We now define a class named `rcpp_proposal` which will be exposed as an Rcpp module. This class inherits from `t_proposal`. (I.e., `rcpp_proposal` "is a" `t_proposal`.) Its constructor, `refine` operation, and `draw` operation are analogous to those in @sec-persistent-xptr.
...
```
1. `t_proposal::refine` is the method in the base class; it is possible to expose this directly from the module, but we opt to call it from `rcpp_proposal::refine` for simplicity.
2. The `supp` function handles most of the constructor work; it returns a single region `supp` with all of the necessary operations. The `rcpp_proposal` constructor creates an instance of itself from region.

The following code exposes `rcpp_proposal` as an Rcpp module.
...
::: {#rem-capture-value-2}
The lambdas `w`, `df`, `pf`, and `qf` in this code capture `kappa` and `d` by value as in @sec-persistent-xptr, which can lead to unwanted overhead from copying mentioned in @rem-capture-value. In the present case, we can consider storing a single copy of any large objects as member variables of the `rcpp_proposal` class and capturing them by reference (rather than by value) in the lambdas.
\remarkqed
:::

# Example: Privacy Protected Data and Sampling within Gibbs {#sec-privacy-within-gibbs}

This section presents an example using the self-tuned VWS method described in @sec-self-tune. Consider again the setting from @sec-privacy and suppose there are $n$ tabulations. Let $y_i$ be the $i$th sensitive tabulation, $z_i$ be its privacy-protected counterpart, and $\vec{x}_i \in \mathbb{R}^d$ be a non-sensitive covariate which is associated with $y_i$. Consider the following hierarchical Bayesian model:
<!-- -->
\begin{align*}
&z_i \sim \text{N}(y_i, \lambda_i^2), \\
&y_i \sim \text{Lognormal}(\vec{x}_i^\top \vec{\beta}, \sigma^2), \\
&\vec{\beta} \sim \text{N}(\vec{0}, \sigma_\beta^2 \vec{I}), \\
&\sigma^2 \sim \text{IG}(a_\sigma, b_\sigma),
\end{align*}
<!-- -->
for $i = 1, \ldots, n$, where $\sigma_\beta^2$, $a_\sigma$, and $b_\sigma$ are hyperparameters which are assumed to be known. The variances $\lambda_i^2$ are assumed to be known and provided with the data. Denote $\vec{\Lambda} = \text{Diag}(\lambda_1^2, \ldots, \lambda_n^2)$ and $\log \vec{y} = (\log y_1, \ldots, \log y_n)$. To obtain a Gibbs sampler, the joint distribution of all random variables is
<!-- -->
\begin{align}
f(\vec{z}, \vec{y}, \vec{\beta}, \sigma^2) =
f_\text{N}(\vec{z} \mid \vec{y}, \vec{\Lambda}) \cdot
f_\text{N}(\log \vec{y} \mid \vec{X} \vec{\beta}, \sigma^2 \vec{I}) \cdot
f_\text{N}(\vec{\beta} \mid \vec{0}, \sigma_\beta^2\vec{I}) \cdot
f_\text{IG}(\sigma^2 \mid a_\sigma, b_\sigma) \cdot
\prod_{i=1}^n \frac{1}{y_i},
\label{eqn:privacy-joint}
\end{align}
<!-- -->
where $f$ with subscripts "N", "LN", and "IG" denotes the normal, lognormal, and inverse gamma densities respectively. From \eqref{eqn:privacy-joint}, we obtain the following conditionals after some algebra:

1. $[\vec{\beta} \mid \sigma^2, \vec{y}, \vec{z}] \sim \text{N}(\vec{\vartheta}, \vec{\Omega}^{-1})$ where $\vec{\Omega} = \sigma^{-2}\vec{X}^\top \vec{X} + \sigma_\beta^{-2} \vec{I}$ and $\vec{\vartheta} = \sigma^{-2} \vec{\Omega}^{-1} \vec{X}^\top (\log \vec{y})$.

2. $[\sigma^2 \mid \vec{\beta}, \vec{y}, \vec{z}] \sim \text{IG}(a', b')$ where $a' = a_\sigma + n/2$ and $b' = b_\sigma + \lVert \log \vec{y} -\vec{X} \vec{\beta} \rVert^2 / 2$.

3. $[\vec{y} \mid \vec{\beta}, \sigma^2, \vec{z}] = \prod_{i=1}^n [y_i \mid \vec{\beta}, \sigma^2, \vec{z}]$ where
<!-- -->
\begin{math}
f(y_i \mid \vec{\beta}, \sigma^2, \vec{z}) \propto f_\text{N}(z_i \mid y_i, \lambda_i^2) \cdot
f_\text{LN}(y_i \mid \vec{x}_i^\top \vec{\beta}, \sigma^2).
\end{math}

Steps 1 and 2 can make use of well-known normal and inverse gamma variate generation methods. For step 3, we will use VWS with a constant majorizer and minorizer as in @sec-privacy-const-custom; these can be readily obtained without numerical optimization. 

@sec-privacy-within-gibbs-r presents a version of the code where the Gibbs sampler is written in R with Rcpp modules to interact with VWS proposals written in C++, as in @sec-persistent-module. @sec-privacy-within-gibbs-cpp presents an alternative version where the Gibbs sampler is written in C++ rather than R. The latter is substantially faster because it avoids the overhead of repeated calls between R and C++; however, it requires more C++ programming and cannot be debugged as readily as the former. Several timings are mentioned; they were measured in Windows using a laptop with a Intel Core i7-1185G7 3.0GHz CPU with four CPU cores.

## Gibbs Sampler in R {#sec-privacy-within-gibbs-r}

This version of the code implements the Gibbs sampler in R and utilizes VWS via Rcpp. We consider two implementations of VWS. One uses persistent proposals that are updated with current parameters from the Gibbs sampler. It makes use of the self-tuning method from @sec-self-tune. The other implementation uses the more basic method of constructing new proposals each time they are needed. Code for this example is given in the folder `examples/privacy-gibbs/v1-R`.

The function `draw_ln_norm` in `draw-ln-norm.cpp` draws a variate from the conditional distribution $[y_i \mid \text{---}]$ using the current values of $z_i$, $\lambda_i$, $x_i^\top \vec{\beta}$, and $\sigma^2$. The remaining arguments are specific to VWS: `tol` and `N` correspond to the tolerance and maximum number of knots in Algorithm \ref{alg:refine}; `max_rejects` is the maximum number of rejections permitted in the `rejection` function. The return value is a list with the contents of a `rejection_result<double>`.
...
The class `ln_norm_proposal` in `ln-norm-proposal.cpp` is an `fmm_proposal` which defines a few additional functions that are exposed to R and contains state that should persist between calls from R. The interface of the class is as follows.
...
public:
	...
	vws::rejection_result<double> draw(       // <3>
		unsigned int max_rejects
	) const;

	vws::rejection_result<double> draw_tune(  // <4>
		double tol_suff,
		double tol_merge,
		unsigned int max_rejects
	);
...
```
1. Constructor from initial values of $z_i$, $\lambda_i$, $x_i^\top \vec{\beta}$, and $\sigma^2$.
2. Update the proposal with values of $x_i^\top \vec{\beta}$ and $\sigma^2$. These values change at each iteration of the Gibbs sampler. The proposal must update its representation of weight function $w$ with these values and recompute the majorizer and minorizer to be an envelope for rejection sampling.
3. Generate a single draw using the `rejection` function.
4. Generate a single draw and self-tune using the `rejection_tune` function. 
5. Return the number of regions in the proposal.

The following snippet in `ln-norm-proposal.cpp` defines an Rcpp Module which exposes functions to R.
...
The file `gibbs.R` contains R code for the Gibbs sampler and some helper functions. See the Roxygen comments that accompany each function for a brief description.

The script `driver.R` generates a dataset from the model and illustrates several calls to the Gibbs sampler. The following snippet uses transient proposals via `draw_ln_norm` (`method = "vws-basic"`), with some details omitted.
...
Note that the setting `tol_suff = 0.85` ensures that each proposal is tuned to have a rejection rate no larger than 85% before rejection sampling. The following snippet illustrates self-tuned VWS using `ln_norm_proposal` (`method = "vws-tune"`).
...
The settings in `control_inner` select the self-tuning method with adjustments made during the first 100 iterations of the MCMC, using $\epsilon_1 = 0.85$ and $\epsilon_2 = 0.01$. See the file `driver.R` script for details.

A particular run of both methods yielded elapsed times of 30 seconds for `vws-basic` and 25 seconds for `vws-tune`. Much of the gain in efficiency due to self-tuning is negated by the overhead from frequent calls between R and C++. This could be mitigated by passing the list of $n$ proposals to C++ at each iteration of the Gibbs sampler. The following section avoids the overhead further by implementing the Gibbs sampler in C++.

## Gibbs Sampler in C++ {#sec-privacy-within-gibbs-cpp}

This version of the code implements the Gibbs sampler in C++ so that it is not necessary to expose VWS proposals to R. Code for this example is given in the folder `examples/privacy-gibbs/v2-cpp`.

The class `ln_norm_proposal` in `ln-norm-proposal.h` defines a VWS proposal as follows.
...
This version of `ln_norm_proposal` is simpler than the one in @sec-privacy-within-gibbs-r. When programming in C++, we can directly make use of the fact that it inherits from `fmm_proposal` as a base class. For example, we can call the `fmm_proposal::r` method to generate draws. We can also pass objects of class `ln_norm_proposal` to the `rejection` or `rejection_tune` functions.

The files `gibbs.cpp` and `gibbs.R` contain code for the Gibbs sampler and helper functions: `gibbs.cpp` implements the Gibbs sampler itself in C++ using RcppArmadillo [@RcppArmadillo] for matrix algebra, while helper functions are implemented in R in `gibbs.R`. Refer to the Roxygen comments in `gibbs.R` for details.

The script `driver.R` generates a dataset from the model and runs two versions of the Gibbs sampler, `"vws-basic"` and `"vws-tune"`, as in @sec-privacy-within-gibbs-r. Here, a run of each yielded elapsed times of 28 seconds for `vws-basic` and 6.2 seconds for `vws-tune`. This represents a reduction of $(28 - 6.2) / 28 = 77.9\%$ compared to $(30 - 25) / 30 = 16.67\%$ seen in @sec-privacy-within-gibbs-r.
