...
- `w_major` and other functions whose arguments are support points: they should
  probably take a list of points to be consistent, right?
:::
...
::: {.callout-note title="TBD"}
- The package is available from <https://github.com/andrewraim/vws> and
  possibly on CRAN(?).
...
Figure \ref{fig:diagram} displays a diagram of the high-level design just described. We will now walk through the components in more depth; however, the user should consult manual entries (e.g., `?Region`) for details such as arguments to methods and their default values.
...
Method | Description
:-----|:-----
| `initialize`       | A standard `R6` constructor that allows objects to be created via `FMMProposal$new(...)`. |
 | `r`  | Generate draws from finite mixture. |
| `d`  | Evaluates the finite mixture density. |
| `nc` | Evaluate normalizing constant $\psi_N$ of the finite mixture density. |
| `rejection_bound` | Returns either the bound \eqref{eqn:bound} or a vector \eqref{eqn:bound-components} which represents the contribution of each region to the bound. |
| `print` | Provides a meaningful display for objects of the class when printed. |
| `summary` | Supports the `print` method. |
| `d_target_unnorm`  | Computes the unnormalized log-density $\log h_0(x) = \log \overline{w}(x) + \log g(x)$ for $x \in \mathcal{D}_j$ which is used as an envelope in rejection sampling.  |
...
| `get_bifurcatable` | Returns a vector of logical values indicating whether respective regions can be bifurcated (described in Section [-@sec-adapt]). |
| `get_regions`      | Returns the list of `Region` objects on which the `FMMProposal` was constructed.|
...
The methods are briefly described in the following table. Expressions with an index $j$ correspond to a $\mathcal{D}_j$ which is associated with the `Region` object.
...
| `w_maj` | The majorized weight function $\overline{w}_j(x)$. |
| `d_base` | Compute the density function $g$ of the base distribution. |
| `r` | Generate draws from truncated and reweighted distribution $g_j$. |
| `d` | Compute the density of truncated and reweighted distribution $g_j$. |
| `s` | Indicator of whether a given point is in the support of truncated and reweighted distribution $g_j$. |
| `xi_upper` | Compute $\overline{\xi}_j$. |
| `xi_lower` | Compute $\underline{\xi}_j$. |
| `bifurcate` | Used with `adapt` discussed in Section [-@sec-adapt]. Bifurcates region object into two new `Region` objects. |
| `is_bifurcatable` | Used with `adapt`. Indicator of whether the region is eligible to be bifurcated. |
| `print` | Provides a meaningful display for objects of the class when printed. |
...
- `Region` abstract class and how to override. Maybe easier to show by example
  later.
:::
...
exponentiation and may be helpful for this purpose; they are listed in the
following table.
...
:-----|:-----
|`log_sum_exp(x)`     | Compute $\log\{ \sum_{i=1}^n \exp(x_i) \}$ based on a vector `x`.|
|`log_add2_exp(x, y)` | Compute $\log(e^x + e^y)$. If `x` and/or `y` is a vector, the function produces a vector of corresponding results.|
|`log_sub2_exp(x, y)` | Compute $\log(e^x - e^y)$; vectorized similar to `log_add2_exp`. Here, elements of `x` smaller than `y` result in `NaN` outputs. |
...
$f(x) = e^{-(x + e^{x})}$. See the functions specified in the following table.

:  {tbl-colwidths="[20,80]"}

Function | Description
:-----|:-----
|`r_categ`      | Generate draws from discrete distribution with values $1, \ldots, k$ and corresponding probabilities (unnormalized, specified on the log-scale) $p_1, \ldots, p_k$ via Gumbel trick.|
...
## Lognormal-Normal Conditional Distribution

::: {.callout-note title="TBD"}
Under construction. What to do here?

- [x] Briefly explain the motivation.
- [x] Show the target (is it still the conditional that we want)?
- [x] Show the decomposition.
- [x] Do the most naive sampler using `UnivariateConstRegion`.
- [x] Try to do a little better and override the optimize function.
- [ ] Try to do even better and use a linear majorizer.
- [ ] We might want to compare the samplers: 
	+ [ ] Run time of numerical versus closed-form optimize;
	+ [ ] Efficiency of const versus linear majorizers.

TBD: why are the first two samplers so slow? They seemed much faster before. Did
I break something in the code?

TBD: maybe we should show more detail in this first example and less in
subsequent examples. If we do, let's inform the reader.
:::

The setting $Z = Y + \xi$ is the basis of a modeling scenario considered by @DirectSamplingDAS2021 and @DPSimulation2022 under privacy protection. Here, $Y$ represents sensitive underlying data such as a tabulation of respondent data and $\xi$ is random noise purposefully added via a privacy protection mechanism. The resulting $Z$ is considered protected. The objective is to carry out inference on $Y$ given an observed $Z = z$. The field of differential privacy studies mathematical criteria for privacy and the design of noise mechanisms which can satisfy those criteria (TBD: cite). To demonstrate a simple but nontrivial sampling problem, suppose $Y$ and $\xi$ are independently distributed with $Y \sim \text{LogNormal}(\mu, \sigma^2)$ and $\xi \sim \text{N}(0, \lambda^2)$. The variance $\lambda^2$ of the noise mechanism is often known and provided with the noisy data under differential privacy. We will also assume that $\mu$ and $\sigma^2$ are known, though in practice these would need to be learned from an observed sample $z_1, \ldots, z_n$.
...
Before proceeding, let us fix the following values.
...
### Constant VWS with Numerical Optimization {#sec-ln-norm-const}
...
Let us code the target density to assist in evaluating sampler results later. Numerical integration is used to obtain the normalizing constant.

```{r}
int_out = integrate(lower = 0, upper = Inf, f = function(x) { 
	w(x, log = FALSE) * helper$d(x, log = FALSE)
})
d_target = function(x, log = TRUE) {
	out = w(x, log = TRUE) + helper$d(x, log = TRUE) - log(int_out$value)
	if (log) { return(out) } else { return(exp(out)) }
}
```
...
The following plot shows the rate of decrease in bound \eqref{eqn:bound} over `N` steps of the adapt call. Let us also print the final bound that was obtained.
...
```{r}
ctrl = rejection_control(report = 2000, extra_outputs = TRUE)
...
### Constant VWS with Custom Optimization {#sec-ln-norm-const-cf}

The weight function for this problem can be either maximized or minimized in closed form. On the log-scale with $\phi(x) = \log w(x)$, we have
<!-- -->
\begin{align*}
\phi(x) = -\log x - \frac{(\log x - \mu)^2}{2 \sigma^2} + \log \mathbb{1}_{(0,\infty)}(x)
\end{align*}
<!-- -->
...
ctrl = rejection_control(report = 2000)
out = rejection(h, n = 10000, control = ctrl)
x = unlist(out)
```

### Linear VWS {#sec-ln-norm-linear}

::: {.callout-note title="TBD"}
Furthermore, it is seen that
<!-- -->
\begin{align*}
&\phi''(x) = -\frac{1}{x^2}\left(
...
\end{align*}
<!-- -->
so that $\phi(x)$ is concave when $x < \exp(\mu - \sigma^2 + 1)$ and convex when $x > \exp(\mu - \sigma^2 + 1)$.
:::
...
h_init = FMMProposal$new(regions)
adapt_out = adapt(h_init, N = 30, report = 1)
h = adapt_out$h
...
ggplot(df) +
	geom_line(aes(x, log_h_vals), lty = 1) +
	geom_line(aes(x, log_f_vals), lty = 2, col = "blue") +
	theme_minimal()
```
...
adapt_out = adapt(h_init, N = 100, control = ctrl)
h = adapt_out$h
```
