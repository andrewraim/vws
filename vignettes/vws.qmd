...
6. Indicator of whether this region is bifurcatable into two smaller regions. This is used when refining a proposal; see @sec-vws-knots. One reason that a region should not be bifurcated is when one of the resulting regions will not have any points in the support. **TBD: delete?**
...
Several `set` functions are provided to update the weight function and univariate helper which represents the base distribution. The third form updates both in the same call, to eliminate some overhead of calling both the first two forms.

```{cpp}
void set(const dfdb& w)
void set(const univariate_helper& helper)
void set(const dfdb& w, const univariate_helper& helper)
```
...
Steps 1 and 2 can make use of well-known normal and inverse gamma variate generation methods. For step 3, we will make use of VWS with a constant majorizer and minorizer as in @sec-privacy-const-custom; the maximizer and minimizer can be readily obtained without numerical optimization. 

@sec-privacy-within-gibbs-r presents a version of the code where the Gibbs sampler is written in R and Rcpp modules are used to interact with VWS proposals written in C++, as in @sec-persistent-module. @sec-privacy-within-gibbs-cpp presents an alternative version of the code where the Gibbs sampler is written in C++ rather than R. The latter is substantially faster because it avoids the overhead of repeated calls between R and C++; however, it requires more C++ programming and cannot be debugged as readily as the first version in R.

Several timings are mentioned; they were measured in Windows using a laptop with a Intel Core i7-1185G7 3.0GHz CPU with four CPU cores.

## Gibbs Sampler in R {#sec-privacy-within-gibbs-r}

This version of the code implements the Gibbs sampler in R and utilizes VWS via Rcpp. We implement two versions of VWS. One version uses persistent proposals that are updated with current parameters from the Gibbs sampler and make use of the self-tuning method from @sec-self-tune. The second version uses the more basic method of constructing new proposals each time they are needed. Code for this example is given in the folder `examples/privacy-gibbs/v1-R`.
...
The class `ln_norm_proposal` in `ln-norm-proposal.cpp` is a proposal which defines a few additional functions that are exposed to R and contains state that will persist between R calls. The interface of the class is as follows.
...
The settings in `control_inner` select the self-tuning method with adjustments made during the first 100 iterations of the MCMC, using $\epsilon_1 = 0.85$ and $\epsilon_2 = 0.01$. See the file `driver.R` script for details. A particular run of each yielded elapsed times of 30 seconds for `vws-basic` and 25 seconds for `vws-tune`. Much of the gain in efficiency due to self-tuning is negated by the overhead from frequent calls from R to C++. This could be mitigated by passing the list of $n$ proposals to C++ at each iteration of the Gibbs sampler. The following section avoids the overhead further by implementing the Gibbs sampler in C++.
...
This version of `ln_norm_proposal` is simpler than the one in @sec-privacy-within-gibbs-r. When programming in C++, we can directly make use of the fact that it inherits from `fmm_proposal` as a base class. For example, we can call the `fmm_proposal::r` method to generate draws and pass `ln_norm_proposal` objects to the `rejection` or `rejection_tune` functions.
