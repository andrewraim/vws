...
**TBD:** for the majorizers, let's get the actual parameters for the constant or linear functions so we can plot them as functions rather than points.
...
\end{align*}
<!-- -->
where $T \sim \text{Poisson}(\lambda^2 / 4)$, and similarly,
<!-- -->
\begin{align*}
\underline{\xi}_j
%
...
\end{align*}
<!-- -->
The proposal $h$ is a finite mixture $h(x) = \sum_{j=1}^N \pi_j g_j(x)$ with
<!-- -->
\begin{align*}
g_j(x)
&= \overline{w}_j(x) g(x) \ind\{ x \in \mathcal{D}_j \} / \overline{\xi}_j \\
