...
Mention `density`, `cdf`, etc definitions from `fntl`.
...
- We may need to provide an implementation of `IntUnivariateConstRegion`, with
  documentation and at least a brief example. If we need this, do we also need
  to provide another R interface function?
...
Here, `region1` and `region2` are objects whose class is a subclass of `Region`. The `Region` class itself is abstract and defines necessary operations. Further details are given in Sections [-@sec-region], [-@sec-UnivariateConstRegion], and [-@sec-user-region].
...
@fig-software-design displays a diagram of the high-level design just described. Sections [-@sec-fmm-proposal] through [-@sec-utilities] will walk through the components in more depth. The user may also consult manual entries (e.g., `?Region`) for details such as arguments to methods and their default values.
...
## rejection_control function {#sec-r-rejection-control}
...
## univariate_helper function {#sec-r-univariate-helper}
...
Sections [-@sec-univariate-const-region] and TBD.
