...
\end{align*}
<!-- -->
C++ code for $\text{Exp}_{(a,b)}(\kappa)$ distribution functions is given in the file `examples/vmf/texp.h`.
:::
...
- Line 3 includes `texp.h`, which provides functions described in @def-line.
...
- Lines 13-20 define the weight function using C++ lambda syntax. We are
  careful to avoid `nan` values that can occur when $x = \pm 1$.
...
- Line 34 constructs a `FMMProposal` based on our initial region `supp`.
  The syntax `{supp}` represents a vector of one region.
...
- Lines 39-43 assemble an `Rcpp::List` to return to the called. It contains 
  the draws (`draws`), a vector of rejection counts `rejects` where the `i`-th
  element represents the number of rejections for the `i`-th draw, and a vector
  (`lbdd`) with the $N-1$ bounds \eqref{eqn:bound-components} achieved at each
  of the refinement steps (returned the log-scale).

**TBD:** add another constructor for `FMMProposal` that takes one region. This
is the most common usage so it would be nice to have a function.

**TBD**: maybe `refine` should take a `log` argument or just return on the
original scale?

We may invoke the exported R function as follows.

```{r}
Rcpp::sourceCpp("examples/vmf/vmf-v1.cpp")
out = r_vmf_pre_v1(n = 1000, kappa = 5, d = 4, N = 10)
head(out$draws)
```
...
@fig-vmf-const-draws plots the empirical distribution of the draws.
...
It is not difficult to find the min and max of the function $w$ on a given interval for this example. We can reduce some of the computational burden by providing functions to compute them.
...
The following snippet creates pointers `maxopt` and `minopt` for the maximizer and minimizer functions. We check if $d > 3$ to determine which is which.
...
Finally, we create the initial region `supp` with the custom optimization functions.
...
The complete example is given in `examples/vmf/vmf-v2.cpp`.

```{r}
Rcpp::sourceCpp("examples/vmf/vmf-v2.cpp")
out = r_vmf_pre_v2(n = 1000, kappa = 5, d = 4, N = 10)
head(out$draws)
```
...
```{.cpp include="examples/ln-norm/ln-norm-v1.cpp" code-line-numbers="true"}
```


**TBD:** Do we need anything below?
...
`w`               $w(y) = \frac{1}{y} \exp\left\{ -\frac{1}{2\sigma^2}
                  (\log y - \mu)^2 \right\} \ind\{ y \in (0,\infty) \}$.
...
&\propto 
\frac{e^{a^2/4} (a/2)^\nu}{\Gamma(x + \nu + 1)}
\frac{(a/2)^{2x} e^{-a^2/4}}{x!} \ind\{ x \in \mathbb{N} \} \\
%
&\propto w(x) g(x),
\end{align*}
<!-- -->
where $g(x) = (a^2/4)^x e^{-a^2/4} \cdot \ind\{ x \in \mathbb{N} \} / x!$
