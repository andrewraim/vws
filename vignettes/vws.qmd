...
**TBD:** Mention density, cdf, quantile function definitions and numerical tools.

## Exposing Functions to R

**TBD:** The model of coding in C++ and exposing functions to R
...
**TBD:** Perhaps this should be pointed out in the first C++ example instead.
...
```{.cpp include="examples/first-cpp/sample2.cpp" code-line-numbers="true"}
```

```{r}
Rcpp::sourceCpp("examples/first-cpp/sampler2.cpp")
...
`UnivariateHelper` is a class which is intended for use with `RealConstRegion`. It contains several operations needed from the base distribution $g$. These operations are specified as lambdas in the constructor.
...
::: {.callout-caution title="TBD"}
- [ ] Why are the first two samplers so slow to refine? They seemed much faster
...
We now create a subclass `CustomConstRegion` of `RealConstRegion` and specify an `optimize` method using the closed-form solution. Furthermore, we specify an `initialize` method which simply invokes `initialize` from the superclass `RealConstRegion`. Otherwise, the behavior of `CustomConstRegion` is identical to `RealConstRegion`.

```{r}
CustomConstRegion = R6::R6Class(

classname = "CustomConstRegion",
inherit = RealConstRegion,
...
From here, the code to construct, refine, and sample from a proposal using `CustomConstRegion` is identical to `RealConstRegion`.
