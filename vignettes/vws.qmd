...
- Briefly explain the motivation.
- Show the target (is it still the conditional that we want)?
- Show the decomposition.
- Do the most naive sampler using `UnivariateConstRegion`.
...
### Constant VWS with Numerical Optimization
...
support = UnivariateConstRegion$new(a = 1e-4, b = Inf, w = w, g = helper)
...
### Constant VWS with Custom Optimization
...
h_init = FMMProposal$new(regions)
adapt_out = adapt(h_init, N = 50, report = 1)
h = adapt_out$h
...
ctrl = rejection_control(report = 2000, max_rejects = 100000)
out = rejection(h, n = 10000, control = ctrl)
x = unlist(out)
