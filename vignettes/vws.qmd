...
The following code builds the complete example and invokes the sampler.
...
We noted in @sec-vmf-const-custom that $w$ is log-convex when $d < 3$, log-concave when $d > 3$, and a constant otherwise. Therefore, we can majorize $w$ with exponentiated linear functions of the form $\overline{w}_j = \exp\{ \overline{\beta}_{j0} + \overline{\beta}_{j1} x \}$. 
...
```{r}
x_star = exp(mu - sigma^2)
w_star = sigma^2 / 2 - mu
printf("Maximizer x = %g obtains value log w(%g).\n", x_star, w_star)
```
...
#|   maximizer $x^* = \exp(\mu - \sigma^2)$ highlighted.
#| label: fig-ln-norm-weight
#| echo: false

xlim = x_star + c(-10,10)
w_one = function(x, log = T) {
	out = -Inf
	if (x > 0) { out = -log(x) - (log(x) - mu)^2 / (2*sigma^2) }
	if (log) { return(out) } else { return(exp(out)) }
}
w = Vectorize(w_one, vectorize.args = "x")
ggplot() +
	geom_function(fun = w, xlim = xlim) +
	geom_vline(xintercept = x_star, lty = 2) +
...
out = r_ln_norm_v2(n = 1000, z, mu, sigma, lambda, N = 50, tol = 0.10)
head(out$draws)
```
