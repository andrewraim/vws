...
1. Examples are not being migrated to the installed package anymore ...
1. Write out the formulation for the linear majorizer.
1. Need an abstract.
1. Need material for `IntConstRegion` in API section.
...
\end{align*}
<!-- -->
respectively. The $g_j$ are truncated and reweighted versions of base distribution $g$. In addition to the majorizer, suppose that $\underline{w}_j$ is a minorizer of $w$ so that
...
rejection sampling to obtain `n` draws. The `rejection` function returns a vector of accepted draws along with information about the number of rejections. @fig-software-design displays a diagram of the high-level design just described.
...
The typedef `dfdd` is defined as a shorthand for a Standard Template Library (STL) [`function`][stl-fun] which takes two `double`s as arguments and returns a `double`. An STL `function` can be passed to other functions like any other variable. Function `f1` explicitly states the return type of the lambda using `-> double` while it is left to be inferred in the others. The variable `z` is captured by `f1`, `f2`, and `f3` so that it is "baked in" to the function definition without subsequently needing to be passed. Functions `f1` and `f2` capture `z` by reference while `f3` captures `z` by value, indicated using the notations `[&]` and `[=]` respectively. Function `f4` uses the notation `[]` to indicated that it does not capture any outside variables.

The `fntl` package defines several typedefs used in the `vws` package, and also provides access to numerical tools such as integration, differentiation, and optimization which take STL `function`s as arguments.
\remarkqed

[stl-fun]: https://en.cppreference.com/w/cpp/utility/functional/function.html
:::

::: {#rem-logscale}
Many of the computations in this vignette and within the `vws` package are carried out on the logarithmic scale. This is to avoid loss of precision when working with very large or small magnitude numbers. Several functions to support log-scale arithmetic are given in @sec-cpp-api-logscale.
\remarkqed
:::
...
This is used when refining a proposal; see @sec-vws-knots. One reason that a
...
**TBD**: When the support is integer-valued, we instead consider
<!-- -->
\begin{align*}
\text{midpoint}(a,b) =
\begin{cases}
...
\end{align*}
<!-- -->
**(Do we still even use this?)**
The function `is_bifurcatable` returns `false` for regions whose width is smaller than 1.
...
them to the density. @fig-vmf-refine plots the bound for the rejection probability for this sampler after each step of refining, along with those from the previous two versions. A substantial improvement in efficiency is seem here; fewer regions are needed to achieve a small rejection probability. Although versions 1 and 2 of the sampler are based on the same proposal, the changes to their bounds are seen to differ due to randomness in knot selection.
...
\end{align*}
<!-- -->
where $T \sim \text{N}(\mu + t \sigma^2, \sigma^2)$. The special case $a = -\infty$ and $b = \infty$ yields the moment-generating function $M(t) = \exp(\mu t + t^2 \sigma^2 / 2)$ of $Y$.
\remarkqed
:::
...
them to the density. @fig-ln-norm-refine plots the bound for the rejection probability for this sampler after each step of refining, along with those from the previous two versions. A substantial improvement in efficiency is seem here; fewer regions are needed to achieve a small rejection probability. Although versions 1 and 2 of the sampler are based on the same proposal, the changes to their bounds are seen to differ due to randomness in knot selection.
...
@fig-bessel-linear-draws plots the empirical distribution of the draws and compares
them to the density. @fig-bessel-refine plots the bound for the rejection probability for this sampler after each step of refining, along with those from the previous two versions. A substantial improvement in efficiency is seem here; fewer regions are needed to achieve a small rejection probability. Although versions 1 and 2 of the sampler are based on the same proposal, the changes to their bounds are seen to differ due to randomness in knot selection.
