...
- Method `d_target_unnorm` computes the unnormalized log-density
  $\log h_0(x) = \log \overline{w}(x) + \log g(x)$ for $x \in \mathcal{D}_j$ which is
...
::: {.callout-note title="TBD"}
What to write about in here?

a. `UnivariateConstRegion`
	- It is a subclass `Region`
	- How to construct one, including `univariate_helper`
b. `IntUnivariateConstRegion`
c. `optimize`: the default method and how to override
:::
...
::: {.callout-note title="TBD"}
- Many of the calculations are done on the log-scale. Describe the log-sum-exp
  functions.
- Gumbel trick to draw from the proposal. (Does this preclude doing a binary
  search though?)
:::
