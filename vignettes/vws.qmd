...
Figure \ref{fig:design} displays a diagram of the high-level design just described. Sections [-@sec-FMMProposal] through [-@sec-utilities] will walk through the components in more depth. The user may also consult manual entries (e.g., `?Region`) for details such as arguments to methods and their default values.
