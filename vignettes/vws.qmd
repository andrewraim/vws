...
```{r}
#| out-width: 60%
#| fig-cap: Proposal versus the target on the log-scale for Bessel.
...
TBD: create two initial regions around the point $\exp(\mu - \sigma^2 + 1)$, where $w(x)$ switches from log-concave to log-convex.
...
TBD: construct proposal and refine it.
...
TBD: plot the rate of refinement.

```{r}
#| out-width: 60%
#| fig-cap: Empirical distribution versus target for Lognormal-Normal.
...
TBD: print the actual rejection rate.
...
TBD: compare the empirical distribution of the draws to the target density.
...
TBD: compare the unnormalized proposal and target density on the log-scale. They are almost indistinguishable.
...
#| out-width: 60%
#| fig-cap: |
#|   Rate of adaptation improvement on the log-scale for VMF constant
#|   majorizer.
...
#| out-width: 60%
#| fig-cap: |
#|   Rate of adaptation improvement on the log-scale for VMF linear majorizer.
