...
- Consider making tables for functions and their arguments, to make browsing
  easier.
...
- Show how to pass in manual list of regions before adapt?
:::
...
Here, `region1` and `region2` are objects whose class is a subclass of `Region`. The `Region` class itself is abstract and defines necessary operations. Further details are given in Sections TBD, TBD, and TBD.
...
| `get_xi_upper`     | Returns $\overline{\xi}_1, \ldots, \overline{\xi}_N$. |
| `get_xi_lower`     | Returns $\underline{\xi}_1, \ldots, \underline{\xi}_N$. |
| `get_bifurcatable` | Returns a vector of logical values indicating whether respective regions can be bifurcated (described further in Section TBD). |
...
| `bifurcate` | Used with `adapt` which is discussed in Section TBD. Bifurcates region object into two new `Region` objects. |
| `is_bifurcatable` | Used with `adapt` which is discussed in Section TBD. Indicator of whether the region is eligible to be bifurcated into two new `Region` objects. |
| `print` | Provides a meaningful display for objects of the class when printed. |
| `description` | Produce a short label that describes the region. |

## Adapting the Proposal

The `adapt` function refines a given proposal, which is based on partition $\mathcal{D}_1, \ldots, \mathcal{D}_{N}$, by sequentially selecting and bifurcating regions $N'$ times to obtain $\mathcal{D}_1, \ldots, \mathcal{D}_{N + N'}$. The following rule of thumb seeks to reduce \eqref{eqn:bound}. Let
<!-- -->
\begin{align}
\rho_\ell = \{
...
## Univariate Regions with Constant Majorizer
...
Function | Description
:-----|:-----
|`log_sum_exp(x)`     | computes $\log\{ \sum_{i=1}^n \exp(x_i) \}$ based on a vector `x`.|
|`log_add2_exp(x, y)` | computes the value $\log(e^x + e^y)$. If `x` and/or `y` is a vector, the functions produces a vector of corresponding results.|
|`log_sub2_exp(x, y)` | computes the value $\log(e^x - e^y)$ and is vectorized similar to `log_add2_exp`. Here, elements of `x` smaller than `y` result in `NaN` outputs.|

We make use of the Gumbel trick (TBD: cite) in several places to draw from a
discrete distribution with values $1, \ldots, k$ and corresponding
