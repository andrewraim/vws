...
The `vws` package facilitates vertical weighted strips (VWS) sampling [@VWS2025]. Namely, the package provides tools to construct proposal distributions for rejection sampling. Variate generation functions are constructed in C++ and may be exposed for use in R via the Rcpp package [@Eddelbuettel2103]. C++ is taken as the primary programming language because of its efficiency as a compiled language and its formal approach to object-oriented programming. The `vws` package has primarily been used to generate from (continuous and discrete) univariate distributions, but is intended to be useable with multivariate distributions as well.
...
**(TBD: where did they go?!)**. Readers looking for a quick start can go directly to the first example in @sec-vmf-const-default whose contents are described in the most detail. Code for the `vws` package is maintained in the GitHub repository at <https://github.com/andrewraim/vws>. 

The remainder of the vignette proceeds as follows. @sec-vws briefly reviews VWS sampling. @sec-prelim gives some preliminaries on programming which are useful for `vws` package users. @sec-overview gives an overview of the package and its major components. @sec-cpp-api provides a detailed specification of the C++ API. @sec-vmf develops several VWS samplers to generate from the Von-Mises Fisher (VMF) distribution which was considered in @VWS2025. @sec-ln-norm develops samplers in a disclosure avoidance setting considered by @DirectSamplingDAS2021 and others. @sec-bessel develops VWS samplers for the Bessel count distribution; variate generation via rejection sampling has been considered in this setting by @Devroye2002.
...
- R version
- C++ version
- `RealConstRegion` is a subclass of `Region` for a particular setting where operations can be coded in a relatively problem-agnostic way.
