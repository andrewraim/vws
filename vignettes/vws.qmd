...
**TBD: write this up nicely**

VWS may be used to generate from conditional distributions in a Gibbs sampler; however, the amount of computation can become laborious if a new proposal (or collection of proposals) is constructed in each step of the Gibbs sampler. In this setting, @saevws-2025 consider a persistent VWS proposal that is gradually tuned to track target a particular conditional distribution of the Gibbs sampler as the MCMC chain advances. If the target does not change too much as the chain approaches the posterior distribution, the number of necessary updates to the proposal should also diminish.

Two types of updates are considered. If bound $\rho_+$ is larger than some threshold $\epsilon_1$, a rejected draw is used to refine the current set of regions. On the other hand, if $\rho_+$ is smaller than $\epsilon_1$, we check for regions with relatively small contributions $\rho_\ell$ to $\rho_+$; such regions are coarsened (i.e., merged with neighboring regions) if doing so does not increase $\rho_+$ beyond $\epsilon_1$.

- $\epsilon_1$: "sufficient" tolerance
- $\epsilon_2$: "merge" tolerance
...
\State Let $N$ be the current number of knots in $h$.
\State Let $\alpha_0, \ldots, \alpha_N$ be the current knots of $h$.
\State Let $\rho_{+}$ be current value of bound \eqref{eqn:bound}.
\State Let $\rho_1, \ldots, \rho_N$ be the current contribution of each region.
\If{$\text{Accept} = 0$ and $\rho_{+} < \epsilon_1$}
\For{$j = 1, \ldots, N-1$}
\If{$\rho_j < \epsilon_2$}
\State Let $\tilde{h}$ be equal to $h$ with knot $\alpha_j$ excluded.
\State Let $\tilde{\rho}_+$ be the bound \eqref{eqn:bound} for $\tilde{h}$.
\State Assign $h$ to $\tilde{h}$ if $\tilde{\rho}_+ < \epsilon_1$.
\EndIf
\EndFor
\ElsIf{$\text{Accept} = 0$ and $\rho_{+} \geq \epsilon_1$}
\State Add $x$ as a knot in $h$.
\EndIf
...
The samplers developed in previous sections each expose a function in R which could be used to draw samples. A proposal is constructed each time the function is called which only exists for the duration of the call. It may be desirable to have a persistent proposal that can be gradually refined and reused. This section demonstrates several Rcpp mechanisms to enable this workflow: first using external pointers them using using modules. The latter requires some object-oriented programming but is safer and recommended over the former. We will demonstrate by revisiting the example from @sec-vmf-const-default. For more information on external pointers and modules, refer to
the vignette "Exposing C++ functions and classes with Rcpp modules" in the `Rcpp` package [@Rcpp] and section "External pointers and weak references" of the guide "Writing R Extensions" [@WritingRExtensions].
...
2. Capture by value with the lambdas here using `[=]`. Variables `kappa` and `d` may not be available in the environment when evaluating the lambdas later; this ensure that they can still be accessed by the lambdas.
3. Create a proposal object on the heap using `new` and wrap it in an `XPtr` to be returned to R.
4. The function `set_tag` a utility we will define; more explanation is provided below.
...
The following typedef provides a more readable names for the proposal.
...
We now define a class named `RcppProposal` which will be exposed as an Rcpp module. This class inherits from ("is a") `t_proposal`. Its constructor, `refine` operation, and `draw` operation are analogous to those in @sec-persistent-xptr.
...
```
1. `t_proposal::refine` is derived from the base class; it is possible to expose this directly from the module, but we opt to call it from `RcppProposal::refine` for simplicity.
