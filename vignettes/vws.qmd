...
Mentioned `density`, `cdf`, etc definitions from `fntl`.

# Older {-}

- If plotting code gets too repetitive, consider making functions for plots.

- We may or may not want to suppress plotting code from the document if it gets
  too lengthy.
...
- Consider changing `UniformHelper` into a container of lambdas so the user
  doesn't need to use inheritance for this part.
...
helper = univariate_helper(
	d = \(x, log = F) { dunif(x, min = lo, max = hi, log = log) },
	s = \(x) { x >= lo & x <= hi },
	p = \(q, lower.tail = T, log.p = F) {
		punif(q, min = lo, max = hi, lower.tail = lower.tail, log.p = log.p)
	},
	q = \(p, lower.tail = T, log.p = F) {
		qunif(p, min = lo, max = hi, lower.tail = lower.tail, log.p = log.p)
	}
)
...
rejection(n = 10, lo, hi, N = 10, tol = 1e-3, w, helper)
```


## First Example in C++ {#sec-first-cpp}

::: {.callout-caution title="TBD"}
Under construction


- A basic program that links to `vws` and does something productive.
- Maybe grab it from the replication materials.
- Explain what is going on in each important line
	a. Including and linking to `vws`.
	a. The `vws` namespace.
- Expose it as an R function.
- Demonstrate how to run it

:::

Let us consider an example C++ code making use of this framework.

```{.cpp include="examples/first-cpp/sampler.cpp" code-line-numbers="true"}
```
...
TBD: here is the code for `UniformHelper.h`.

```{.cpp include="examples/first-cpp/UniformHelper.h" code-line-numbers="true"}
```
...
sample(n = 10, kappa = 5, d = 4, lo = -1, hi = 1, N = 10)
```
...
## (STALE) Univariate Regions with Constant Majorizer {#sec-UnivariateConstRegion}
...
## `rejection` function {#sec-r-rejection}
...
6. Maximum number of regions to create in the proposal.
7. Desired bound for probability of rejection in the proposal.
8. An object constructed using `rejection_control` (@sec-r-rejection-control).
...
#| eval: false
rejection_control(
	max_rejects = .Machine$integer.max,               # <1>
	report = .Machine$integer.max,                    # <2>
	ratio_ub = exp(1e-5),                             # <3>
	action = c("stop", "warning", "message", "none")  # <4>
)
```	
1. Maximum number of rejections to tolerate before bailing out.
2. Report progress each time this many candidates are proposed.
3. Rejection ratio $f_0(x) / h_0(x)$ may be larger than 1 due to numerical
precision. Throw an error if it is larger than this value.
4. Action to take if rejection sampling halts early due to the maximum number
...
The `bifurcate` returns two disjoint regions whose union is the current region.
The result is given as an STL pair.
...
std::pair<UnivariateConstRegion,UnivariateConstRegion>
bifurcate(const double& x) const;                       \\ <2>
```
1. This version partitions the the midpoint of the current region, determined
by the `midpoint` method.
2. This version partitions at the given $x$.
...
"less than" $(a_2, b_2]$ if $a_1 < a_2$. The regions are considered equal if
...
## UnivariateHelper {#sec-univariate-helper}

`UnivariateHelper` is an abstract base class for use with `UnivariateConstRegion`. It encapsulates several operations needed from the base distribution $g$. Users should subclass `UnivariateHelper` with operations for their particular problem.

The class has a template argument `T` which represents the underlying data type.
...
The following methods are abstract and intended to be overridden.

```{cpp}
virtual double d(T x, bool log = false) const = 0;                     // <1>
virtual double p(T q, bool lower = true, bool log = false) const = 0;  // <2>
virtual double q(T p, bool lower = true, bool log = false) const = 0;  // <3>
virtual bool s(T x) const = 0;                                         // <4>
```
