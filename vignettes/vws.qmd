...
Subclasses of `Region` are expected to provide several additional functions. Suppose `MyRegion` is a subclass of `Region` whose elements are variables of type `type` (e.g., `double`); its definition should include the following.
...
```
1. Bifurcate the current region into two regions; this version takes no arguments and includes logic to decide where to make the split.
...
7.  Specify the density, CDF, and quantile function of the base distribution $\text{Exp}_{(-1,1)}(\kappa)$. These are defined as lambdas whose signatures are defined in the `fntl` package. For example, a function of type `fntl::density` takes two inputs: a double `x` which is the argument and `log` which is a boolean that specifies whether the value should be returned on the log-scale.
...
::: {#rem-vmf-endpoints}
This code will fail if we attempt to use it with $d = 2$. Here, $\log w(x) \rightarrow \infty$ as $x \rightarrow \pm 1$ so that the first and last regions cannot be bounded by an exponentiated linear form. This can be averted by truncating the support to $(-1 + \epsilon, 1 - \epsilon]$ for a small enough $\epsilon > 0$.
\remarkqed
:::
...
While Sections [-@sec-vmf] and [-@sec-ln-norm] considered continuous distributions, we now demonstrate generation from a discrete target. Consider generating from the Bessel distribution [@YuanKalbfleisch2000] with density
<!-- -->
\begin{align*}
...
#|   $\log w(x)$ at integers $x \in \{ 0, 1, \ldots \}$.
#| fig-subcap:
#|   - v1.
#|   - v2.
#|   - v3.
