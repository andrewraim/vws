...
With a constant majorizer for $w$, the expressions used in bound \eqref{eqn:bound} are
<!-- -->
\begin{align}
\overline{\xi}_j = \overline{w}_j \Prob(\alpha_{j-1} < T \leq \alpha_{j})
\quad \text{and} \quad
\underline{\xi}_j = \underline{w}_j \Prob(\alpha_{j-1} < T \leq \alpha_{j}),
\label{eqn:vmf-vws-constant-xi}
\end{align}
<!-- -->
where $T \sim \text{Exp}_{(\alpha_{0}, \alpha_N)}(\kappa)$, $\alpha_{0} = -1$,
and $\alpha_{N} = 1$.
