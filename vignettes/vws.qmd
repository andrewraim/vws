...
Before proceeding, let us fix values of $\lambda$ and $\nu$ and compute values of $\log w(x)$ and $f(x)$; these will be useful for illustration.
...
@fig-bessel-const-refine plots the bound for the rejection probability during the refinement process, which is captured in the variable `out$lbdd`.
...
@fig-bessel-const-draws plots the empirical distribution of the draws and compares
them to the target probability mass function (pmf).
...
@fig-bessel-refine plots the bound for the rejection probability for this sampler after each step of refining, along with those from the previous two versions. A substantial improvement in efficiency is seem here; fewer regions are needed to achieve a small rejection probability. Although versions 1 and 2 of the sampler are based on the same proposal, the changes to their bounds are seen to differ due to randomness in knot selection.
...
#| out-width: 60%
#| fig-cap: |
#|   Majorizers (on the log-scale) for Bessel example for versions 1 (left),
#|   2 (center), and 3 (right). The blue curve with is the majorizer with a
#|   blue dot marking the right endpoint of each region. The black triangle
#|   displays the value of $log w$ at integers $0, 1, \ldots$.
...
#| out-width: 60%
#| fig-cap: |
#|   Proposal density (bars) for Bessel example for versions 1 (left),
#|   2 (center), and 3 (right). Triangles display the values of
#|   $f(0), f(1), \ldots$.
#| label: fig-bessel-proposals
#| echo: false

out1 = r_bessel_v1(0, lambda, nu, N, tol, max_rejects*50, report, x = xseq)
out2 = r_bessel_v2(0, lambda, nu, N, tol, max_rejects*50, report, x = xseq)
out3 = r_bessel_v3(0, lambda, nu, lo = -0.1, hi = 1e5, N, tol, max_rejects,
	report, x = xseq)
