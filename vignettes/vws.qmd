...
- Make sure to mention that other region types (e.g., multivariate) can be
...
The following computes $\log\{ \sum_{i=1}^n \exp(x_i) \}$. It yields
a scalar based on a vector $x \in \mathbb{R}^n$.
...
```{.cpp include="examples/vmf-cpp/sample.cpp" code-line-numbers="true"}
```
...
```{r}
Rcpp::sourceCpp("examples/vmf-cpp/sampler.cpp")
...
```{.cpp include="examples/vmf-cpp/sample2.cpp" code-line-numbers="true"}
```

```{r}
Rcpp::sourceCpp("examples/vmf-cpp/sampler2.cpp")
