...
The following remark is useful to formulate the linear majorizer.

::: {#rem-incgamma}
Equation 6.5.13 of @AbramowitzStegun1972 can be rewritten as
<!-- -->
\begin{displaymath}
\sum_{k=0}^{n-1} \frac{x^k}{k!} = e^x \frac{\Gamma(n,x)}{\Gamma(n)},
\quad n \in \{ 1, 2, \ldots \},
\end{displaymath}
<!-- -->
where $\Gamma(a,x) = \int_x^\infty t^{a-1} e^{-t} dt$ and $\Gamma(a) = \Gamma(a,0)$. Therefore, for nonnegative integers $1 \leq r \leq s$,
<!-- -->
\begin{align*}
\sum_{k = r}^{s} \frac{x^k}{k!}
&= \sum_{k = 0}^{s} \frac{x^k}{k!} - \sum_{k = 0}^{r-1} \frac{x^k}{k!} \\
&= e^x \frac{\Gamma(s+1,x)}{\Gamma(s+1)}
- e^x \frac{\Gamma(r,x)}{\Gamma(r)}.
\end{align*}
<!-- -->
...
\begin{math}
\Prob(r < X \leq s) =
\Gamma(s+1, \lambda) / \Gamma(s+1)
-\Gamma(r+1, \lambda) / \Gamma(r+1).
\end{math}
\remarkqed
:::

@rem-incgamma yields the expression
<!-- -->
\begin{align*}
\overline{\xi}_j
...
\sum_{k = \lceil \alpha_{j-1} \rceil}^{\lfloor \alpha_j \rfloor}
...
\left\{
\frac{
\Gamma(\lfloor \alpha_j \rfloor + 1, \frac{\lambda^2}{4} e^{ \overline{\beta}_{j1}})
}{
\Gamma(\lfloor \alpha_j \rfloor + 1)
}
-
\frac{
\Gamma(\lceil \alpha_{j-1} \rceil, \frac{\lambda^2}{4} e^{ \overline{\beta}_{j1}})
}{
\Gamma(\lceil \alpha_{j-1} \rceil)
}
\right\}
\end{align*}
<!-- -->
...
\begin{align*}
\underline{\xi}_j
&= \int_{\alpha_{j-1}}^{\alpha_j} \underline{w}_j(x) g(x) d\nu(x) \\
%
&= \exp\left(
\underline{\beta}_{j0} - \frac{\lambda^2}{4}
+ \frac{\lambda^2}{4} e^{ \underline{\beta}_{j1} } \right)
\left\{
\frac{
\Gamma(\lfloor \alpha_j \rfloor + 1, \frac{\lambda^2}{4} e^{ \underline{\beta}_{j1}})
}{
\Gamma(\lfloor \alpha_j \rfloor + 1)
}
-
\frac{
\Gamma(\lceil \alpha_{j-1} \rceil, \frac{\lambda^2}{4} e^{ \underline{\beta}_{j1}})
}{
\Gamma(\lceil \alpha_{j-1} \rceil)
}
\right\}.
...
\frac{1}{x!} (\lambda^2 / 4)^x e^{-\lambda^2 / 4}
...
\right\}^{-1}
\ind(\lceil \alpha_{j-1} \rceil < x \leq \lfloor \alpha_j \rfloor)
\end{align*}
<!-- -->
which is the density of $\text{Poisson}(\frac{\lambda^2}{4} e^{ \overline{\beta}_{j1}})$ truncated to the interval $(\lceil \alpha_{j-1} \rceil, \lfloor \alpha_j \rfloor]$.

To construct $\overline{\beta}_{j0}$ and $\overline{\beta}_{j1}$, we make use of method (TBD) using the expression
<!-- -->
\begin{align*}
M_j(s)
&= \sum_{\alpha_{j-1}}^{\alpha_j} e^{sx} g(x) d\nu(x) \\
&= \exp( -\lambda^2 / 4 )
\sum_{x = \lceil \alpha_{j-1} \rceil}^{\lfloor \alpha_j \rfloor}
\frac{(e^{s} \lambda^2 / 4)^x}{x!} \\
%
&= \exp\left\{ \frac{\lambda^2}{4} (e^{s} - 1) \right\}
\left\{
\frac{
\Gamma(\lfloor \alpha_j \rfloor + 1, \frac{\lambda^2}{4} e^s)
}{
\Gamma(\lfloor \alpha_j \rfloor + 1)
}
-
\frac{
\Gamma(\lceil \alpha_{j-1} \rceil, \frac{\lambda^2}{4} e^s)
}{
\Gamma(\lceil \alpha_{j-1} \rceil)
}
\right\}.
\end{align*}
<!-- -->
where the last line is obtained using @rem-incgamma.

The following R code builds the complete example and invokes the sampling function.

```{r}
Rcpp::sourceCpp("examples/bessel/bessel-v3.cpp")
out3 = r_bessel_v3(n = 1000, z, mu, sigma, lambda, lo = 1e-8, 1e8,
	N = 50, tol = 0.10)
head(out3$draws)
```
